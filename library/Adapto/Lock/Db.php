 ...
 * @subpackage lock
 */
class Adapto_Lock_Db extends Adapto_Lock
{
    /**
    ...
    public function __construct()
    {
        ...
        for ($i = count($atklock["stack"]); $i < atkLevel(); $i++)
            $atklock["stack"][] = array();
        $this->m_id = (int) $atklock["id"];
    ...
    function unlock($selector, $table)
    {
        ...
        $query
                ->addCondition(
                        "(" . "lock_id = '" . (int) $this->m_id . "' AND " . "lock_table = '" . escapeSQL($table) . "' AND " . "lock_record = '"
                                . escapeSQL($selector) . "' AND " . "user_id = '" . escapeSQL($user) . "' AND " . "user_ip = '" . escapeSQL(atkGetClientIp())
                                . "' AND " . "session_id = '" . escapeSQL(session_id()) . "'" . ") OR (" . "lock_lease <= " . $db->func_now() . ")");
    ...
    function extend($identifier)
    {
        ...
        if (!empty($Adapto_VARS["stack"]))
            $atkstackid = $Adapto_VARS["stack"];
        else
    ...
    function isLocked($selector, $table, $mode = self::EXCLUSIVE)
    {
        ...
        /* select all locks for the entity table -> cache */
