 ...
 */
$config_identifier = Adapto_Config::getGlobal("identifier");
...
if (Adapto_Config::getGlobal('session_init', true) && atksession_init()) {
    ...
    $GLOBALS['g_sessionData'] = &$_SESSION[Adapto_Config::getGlobal('identifier')];
}
...
define("SESSION_PARTIAL", 5); // same as replace, but ignore atkentitytype and atkaction
...
class Adapto_Session_Manager
{
     ...
     *                          publiciable, or "globals" if the global value // defaulted to public
     ...
     * Adapto_Config::getGlobal('session_max_stack_inactivity_period').
     */
    ...
    protected function _removeExpiredStacks()
    {
        ...
        $maxAge = Adapto_Config::getGlobal('session_max_stack_inactivity_period', 0);
        if ($maxAge <= 0) {
    ...
    protected function _stackscope(&$postvars)
    {
        ...
        $sessionVars = array("atkentitytype", "atkfilter", "atkaction", "atkpkret", "atkcontroller", "atkwizardpanelindex", "atkwizardaction", 'atkstore',
                'atkstore_key');
        ...
        $pageVars = array("atkdg*", "atkdgsession", "atksearch", "atkselector", "atksearchmode", "atkorderby", "atkstartat", "atklimit", "atktarget",
                "atkformdata", "atktree", "atksuppress", "atktab", "atksmartsearch", "atkindex");
        ...
        $lockedVars = array('atkentitytype', 'atkaction', 'atkselector');
            ...
            } else if ($atklevel == -3) // SESSION_PARTIAL
 {
            ...
            if (!isset($currentitem) || $currentitem == "") {
                ...
                foreach ($pageVars as $var) {
                    ...
                    if (isset($postvars[$var]) && count($postvars[$var]) > 0 && (!$partial || !in_array($var, $lockedVars))) {
                        if ($recursive && is_array($currentitem[$var]) && is_array($postvars[$var])) {
                            $currentitem[$var] = Adapto_array_merge_recursive($currentitem[$var], $postvars[$var]);
                        } else {
                            $currentitem[$var] = $postvars[$var];
                        }
                    }
            ...
            if (isset($currentitem["atkformdata"]) && is_array($currentitem["atkformdata"])) {
                ...
                foreach ($currentitem['atkformdata'] as $var => $value) {
                    ...
                    if (!isset($postvars[$var])) {
                        ...
                        $postvars[$var] = Adapto_array_merge_keys($value, $postvars[$var]);
                    }
                }
                ...
                // present. However, this data should not be directly accessed in the entity!
            }
    ...
    public function stackTrace()
    {
        ...
        for ($i = 0; $i < count($stack); $i++) {
            if (!isset($stack[$i]["atkentitytype"]))
                continue;

            $entity = $stack[$i]["atkentitytype"];
            ...
            $entry = array('url' => '', 'title' => $title, 'descriptor' => $descriptor, 'entity' => $entity, 'entitytitle' => atktext($type, $module, $type),
                    'action' => $action, 'actiontitle' => atktext($action, $module, $type));
    ...
    /**
     * Gets the entity and the descriptor for the current item
    ...
    public function descriptorTrace()
    {
        ...
        for ($i = 0; $i < $stackcount; $i++) {
            if (isset($stack[$i]["descriptor"]) || $i == ($stackcount - 1)) {
                if ($stack[$i]["atkentitytype"] != "") {
                    $entity = &getEntity($stack[$i]["atkentitytype"]);
                    $module = getEntityModule($stack[$i]["atkentitytype"]);
                    $entityname = getEntityType($stack[$i]["atkentitytype"]);
                }
     ...
     *
     * The advantage of using Adapto_Session_Manager::getSession over php's
    ...
    {
        if (!isset($_SESSION[Adapto_Config::getGlobal("identifier")]) || !is_array($_SESSION[Adapto_Config::getGlobal("identifier")]))
            $_SESSION[Adapto_Config::getGlobal("identifier")] = array();
        return $_SESSION[Adapto_Config::getGlobal("identifier")];
    }
    ...
    public function formState($sessionstatus = SESSION_DEFAULT, $returnbehaviour = NULL, $fieldprefix = '')
    {
        ...
        $newlevel = Adapto_Session_Manager::newLevel($sessionstatus);
    ...
    static public function sessionVars($sessionstatus = SESSION_DEFAULT, $levelskip = null, $url = "")
    {
        ...
        $newlevel = Adapto_Session_Manager::newLevel($sessionstatus, $levelskip);
        $oldlevel = Adapto_Session_Manager::oldLevel($sessionstatus, $levelskip);
    ...
    static public function sessionUrl($url, $sessionstatus = SESSION_DEFAULT, $levelskip = null)
    {
        ...
        $url .= Adapto_Session_Manager::sessionVars($sessionstatus, $levelskip, $url);
    ...
    {
        if ($saveform) {
            $str = 'atkSubmit("' . atkurlencode(Adapto_Session_Manager::sessionUrl($url, $sessionstatus)) . '");';
            return "<a href=\"javascript:void(0)\" onclick=\"" . Adapto_htmlentities($str) . "\" " . $extraprops . ">" . $name . "</a>";
        } else {
            $str = Adapto_Session_Manager::sessionUrl($url, $sessionstatus);
            return "<a href=\"" . Adapto_htmlentities($str) . "\" " . $extraprops . ">" . $name . "</a>";
        }
    }
}
...
function atksession_init()
{
    ...
    $cookiepath = Adapto_Config::getGlobal("application_root");
    $cookiedomain = (Adapto_Config::getGlobal("cookiedomain") != "") ? Adapto_Config::getGlobal("cookiedomain") : NULL;
    ...
    if (isValidSessionId()) {
        $sessionname = Adapto_Config::getGlobal("session_name");
        if (!$sessionname)
            $sessionname = Adapto_Config::getGlobal('identifier');
...
function atksession($namespace = "default", $usestack = true)
{
    ...
    atkDataDecode($Adapto_VARS);
    if (array_key_exists('atkfieldprefix', $Adapto_VARS) && $Adapto_VARS['atkfieldprefix'] != '')
        $Adapto_VARS = $Adapto_VARS[$Adapto_VARS['atkfieldprefix']];

    $g_sessionManager->session_read($Adapto_VARS);

    // Escape check
    if (isset($_REQUEST["atkescape"]) && $_REQUEST["atkescape"] != "") {

        atkEntity::redirect(atkurldecode($_REQUEST["atkescape"]));
    ...
    // Nested URL check
        ...
        atkEntity::redirect(session_url($_REQUEST["atknested"], SESSION_NESTED));
        atkOutput::getInstance()->outputFlush();
        exit;
    }
    // Back check
 else if (isset($Adapto_VARS["atkback"]) && $Adapto_VARS["atkback"] != "") {
        // When we go back, we go one level deeper than the level we came from.

        atkEntity::redirect(session_url(atkSelf() . "?atklevel=" . ($atkprevlevel - 1)));
        atkOutput::getInstance()->outputFlush();
        exit;
    }
}
...
{
    $name = Adapto_Config::getGlobal("session_name");
    if (empty($name))
        $name = Adapto_Config::getGlobal("identifier");
 ...
 * @return Adapto_Session_Manager Session manager
 */
