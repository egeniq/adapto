 ...
 *
 */
class Adapto_Db_Pgsql extends Adapto_Db
{
    /* identification */
    public $m_type = "pgsql"; // defaulted to public
    public $m_vendor = "postgresql"; // defaulted to public
                ...
                if (function_exists("pg_last_error")) // only available since PHP 4.2.0.
 {
    ...
    {
        /* goto next record */
        $this->m_record = @pg_fetch_array($this->m_query_id, $this->m_row, PGSQL_ASSOC | Adapto_Config::getGlobal("pgsqlfetchmode"));
        $this->m_row++;
    ...
    function nextid($sequence)
    {
        /* connect first */
        if ($this->connect() == DB_SUCCESS) {
            $sequencename = Adapto_Config::getGlobal("database_sequenceprefix") . $sequence;
    ...
    function metadata($table, $full = false)
    {
        ...
        foreach ($rows as $i => $row) {
            ...
            $meta[$i]['flags'] = ($row['is_primary'] == 1 ? MF_PRIMARY : 0) | ($row['is_unique'] == 1 ? MF_UNIQUE : 0)
                    | ($row['is_not_null'] == 1 ? MF_NOT_NULL : 0) | ($row['is_auto_inc'] == 1 ? MF_AUTO_INCREMENT : 0);

            if ($row['is_auto_inc'] == 1)
                $meta[$i]['sequence'] = $row['sequence'];

            else if (Adapto_strlen($row['default']) > 0) {
                // date/time/datetime
