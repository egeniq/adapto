 ...
 * @subpackage db.statement
 */
class Adapto_Db_Statement_MySQLi extends Adapto_Statement
{
    /**
     * MySQLi statement.
     * 
    ...
    protected function _prepare()
    {
        ...
        if (!$this->m_stmt || $conn->errno) {
            throw new Adapto_StatementException("Cannot prepare statement (ERROR: {$conn->errno} - {$conn->error}).",
                    atkStatementException::PREPARE_STATEMENT_ERROR);
        }
    }
    ...
    private function _storeColumnNames()
    {
        ...
        if ($this->m_stmt->errno) {
            throw new Adapto_StatementException("Cannot retrieve metadata (ERROR: {$this->m_stmt->errno} - {$this->m_stmt->error}).",
                    atkStatementException::OTHER_ERROR);
        }
    ...
    protected function _execute($params)
    {
        ...
        if (!$this->m_stmt->execute()) {
            throw new Adapto_StatementException("Cannot execute statement: {$this->m_stmt->error}", atkStatementException::STATEMENT_ERROR);
        }
