 ...
 */
require_once(Adapto_Config::getGlobal("atkroot") . "atk/db/class.atkmysqldb.inc");
 ...
 * @subpackage db
 */
class Adapto_Db_Mysqli extends Adapto_MysqlDb
{
    /**
     ...
     * @access protected
     */
    public $m_insert_id; // defaulted to public
    ...
    function query($query, $offset = -1, $limit = -1)
    {
        ...
        if ($this->connect($mode) == DB_SUCCESS) {
            ...
            if (Adapto_Config::getGlobal('debug') >= 1) {
                $this->debugWarnings();
            }
    ...
    {
        /* goto next record */
        $this->m_record = @mysqli_fetch_array($this->m_query_id, MYSQLI_ASSOC | Adapto_Config::getGlobal("mysqlfetchmode"));
        $this->m_row++;
        $this->m_errno = mysqli_errno($this->m_link_id);
        $this->m_error = mysqli_error($this->m_link_id);
