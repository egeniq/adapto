 ...
 *
 */
class Adapto_Util_StringParser
{
    public $m_fields = array(); // defaulted to public
    public $m_string = ""; // defaulted to public
    ...
    function getAllParsedFieldsAsArray($data, $split_tags_and_fields = false)
    {
        ...
        Adapto_var_dump($matches, "MATCHES" . ($split_tags_and_fields ? " (split tags and separators)" : ""));
        ...
        if (is_array($matches)) {
            foreach ($matches[0] as $match) {
                ...
                if (strpos($match, '[') !== false && strpos($match, ']') !== false) {
                    $parser = new Adapto_Util_StringParser($match);
