...
/**
 * Handler for the 'delete' action of an entity. It asks the user for
 ...
 *
 */
class Adapto_Handler_Delete extends Adapto_ActionHandler
{
    ...
    function action_delete()
    {
        ...
        if (!empty($this->m_postvars['confirm']))
            $this->_doDelete();
        elseif (empty($this->m_entity->m_postvars['cancel'])) {
            ...
            // If we got here, then the entity is not locked and we haven't displayed the
            // confirmation page yet, so we display it
            $page = &$this->getPage();
            $page
                    ->addContent(
                            $this->m_entity
                                    ->renderActionPage("delete",
                                            $this->m_entity->confirmAction($this->m_postvars['atkselector'], "delete", false, TRUE, true, $this->getCSRFToken())));
        } else {
    ...
    protected function _handleCancelAction()
    {
        ...
        $location = $this->m_entity->feedbackUrl("delete", ACTION_CANCELLED);
    ...
    function _checkAllowed()
    {
        ...
        $recordset = $this->m_entity->selectDb($atkselector_str, "", "", "", "", "delete");
    ...
    protected function _doDelete()
    {
        ...
        if ($result === true)
            $location = $this->m_entity->feedbackUrl("delete", ACTION_SUCCESS);
        else
            $location = $this->m_entity->feedbackUrl("delete", ACTION_FAILED, null, $result);
    ...
    protected function _doDeleteDb()
    {
        ...
        if ($this->m_entity->deleteDb($this->m_postvars['atkselector'])) {
            $db->commit();
            $this->clearCache();
            return true;
        } else // Something is wrong here, the deleteDb failed
 {
    ...
    /**
     * We check if the entity is locked, if it is, we display the locked page,
    ...
    function _checkLocked()
    {
        ...
        if ($this->m_entity->hasFlag(EF_LOCK)) {
            // We assume that the entity is locked, unless proven otherwise
            $locked = TRUE;
            if (is_array($this->m_postvars['atkselector'])) {
                foreach ($this->m_postvars['atkselector'] as $selector)
                    if (!$this->m_entity->m_lock->lock($selector, $this->m_entity->m_table, $this->m_entity->getLockMode()))
                        $locked = FALSE;
            } elseif (!$this->m_entity->m_lock->lock($this->m_postvars['atkselector'], $this->m_entity->m_table, $this->m_entity->getLockMode()))
                $locked = FALSE;

            // If the entity is locked, we proceed to display the 'locked' page
            if (!$locked) {
                $page = &$this->getPage();
                $page->addContent($this->m_entity->lockPage());
                return true;
            }
        }
    }
    ...
    /**
     * Checks with each of the attributes of the entity whose record is about to be deleted
     * if they allow the deletion
     * @return bool wether or not the attributes have allowed deletion
     */
    function checkAttributes()
    {
        foreach ($this->m_entity->getAttributes() as $attrib) {
            // If allowed !=== true, then it returned an error message
            if ($attrib->deleteAllowed() !== true) {
                $db = &$this->m_entity->getDb();
                $db->rollback();
                $location = $this->m_entity
                        ->feedbackUrl("delete", ACTION_FAILED, null,
                                sprintf(atktext("attrib_delete_not_allowed"), atktext($attrib->m_name, $this->m_entity->m_module, $this->m_entity->m_type), $allowed));
                $this->m_entity->redirect($location);
                return;
            }
        }
