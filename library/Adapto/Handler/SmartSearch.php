...
class Adapto_Handler_SmartSearch extends Adapto_AbstractSearchHandler
{
    ...
    function action_smartsearch()
    {
        ...
        $actionPage = $this->m_entity->renderActionPage("smartsearch", $smartSearchPage);
    ...
    function partialCriteriumField()
    {
        ...
        $path = $this->getEntityAndAttrPath($attrNames);
    ...
    function _partialCriteriumValueOrMode($type)
    {
        ...
        if (count($path) == $field_nr + 1) {
            ...
            return $this->m_entity->text('none');
        }
    }
    ...
    function redirectToResults($criteria)
    {
        ...
        $url = dispatch_url($this->m_entity->atkEntityType(), 'admin', $params, atkSelf());
        $this->m_entity->redirect(session_url($url, atkLevel() == 0 ? SESSION_REPLACE : SESSION_BACK));
    }
    ...
    function getLabels()
    {
        $labels = array('criterium_field', 'criterium_value', 'criterium_mode', 'add_criterium', 'remove_criterium', 'load_criteria', 'save_criteria',
                'forget_criteria', 'reset_criteria');
        $result = array();
        foreach ($labels as $label)
            $result[$label] = Adapto_htmlentities(atktext($label, 'atk'));
        return $result;
    }
    ...
    function getTemplate($name)
    {
        $ui = &$this->getUi();
        if ($name == 'form')
            return $this->m_entity->getTemplate('smartsearch');
        else if ($name == 'criterium')
            return $ui->templatePath('smartcriterium.tpl');
    }

    /**
     * Get searchable attributes for the given entity.
     *
     * @param atkEntity $entity     reference to the entity
     * @param Array   $excludes attribute exclude list
     *
     * @return Array list of reference to searchable attributes
     */
    function getSearchableAttributes(&$entity, $excludes)
    {
        $attrNames = array_keys($entity->m_attribList);
    ...
    function getAttributeList($entry)
    {
        if (count($entry['attrs']) == 1 && !$entry['includeSelf']) {
            $attr = &$entry['attrs'][0];
            $label = is_a($attr, 'atkmanytoonerelation') ? '' : Adapto_htmlentities(strip_tags($attr->label()));
        ...
        if ($entry['includeSelf']) {
            $result .= '<option value="."' . ($entry['selectSelf'] ? ' selected="selected"' : '') . '>' . $this->m_entity->text('self') . '</option>'
                    . '<option value=""></option>';
        }
        ...
        for ($i = 0, $_i = count($entry['attrs']); $i < $_i; $i++) {
            ...
            $label = Adapto_htmlentities(strip_tags($attr->label()));
    ...
    /**
     * Adds an entity/attribute entry to the entity/attribute path.
     *
     * The entry consists of the the following fields:
     * - nr,          number in the entity/attribute path (>= 0)
     * - entity,        reference to the entity for this path entry
     * - attr,        reference to the currently selected attribute for this path entry
     * - attrs,       all searchable attributes for this entity
     ...
     * @param atkEntity $entity        reference to the current entity
     ...
     *
     * @return atkEntity next entity
     */
    function &addEntityAndAttrEntry(&$path, &$entity, $attrName, &$includeSelf, &$excludes)
    {
        $attr = &$entity->getAttribute($attrName);

        $nr = count($path);
        $attrs = $this->getSearchableAttributes($entity, $excludes);
        ...
        $entry = array('nr' => $nr, 'entity' => &$entity, 'attrs' => $attrs, 'attr' => &$attr, 'includeSelf' => $includeSelf, 'selectSelf' => $selectSelf);
        $path[] = &$entry;
    ...
    /**
     * Returns the entity/attribute path for the given
     * attribute name path.
     *
     * @param Array $attrPath attribute name path
     * @return Array entity/attribute path
     */
    function getEntityAndAttrPath($attrPath)
    {
        $path = array();
        ...
        foreach ($attrPath as $attrName) {
            $entity = &$this->addEntityAndAttrEntry($path, $entity, $attrName, $includeSelf, $excludes);
            if ($entity == NULL)
                break;
        }

        while ($entity != NULL) {
            $entity = &$this->addEntityAndAttrEntry($path, $entity, NULL, $includeSelf, $excludes);
        }
    ...
    function getCriteriumField($id, $path, &$scriptCode)
    {
        ...
        for ($i = count($path) - 1, $_i = 0; $i >= $_i; $i--) {
            ...
            if (!$readOnly) {
                ...
                $fieldUrl = session_url(
                        dispatch_url($this->m_entity->atkEntityType(), 'smartsearch',
                                array('criterium_id' => $id, 'field_nr' => $entry['nr'], 'atkpartial' => 'criteriumfield'), atkSelf()), SESSION_NEW);
                $valueUrl = session_url(
                        dispatch_url($this->m_entity->atkEntityType(), 'smartsearch',
                                array('criterium_id' => $id, 'field_nr' => $entry['nr'], 'atkpartial' => 'criteriumvalue'), atkSelf()), SESSION_NEW);
                $modeUrl = session_url(
                        dispatch_url($this->m_entity->atkEntityType(), 'smartsearch',
                                array('criterium_id' => $id, 'field_nr' => $entry['nr'], 'atkpartial' => 'criteriummode'), atkSelf()), SESSION_NEW);
    ...
    function _getCriteriumValueOrMode($id, $path, $value, $mode, $type)
    {
        ...
        if ($attr == NULL) {
            ...
            $attr->addToSearchFormFields($fields, $entry['entity'], $valueArray, $prefix, $modeArray);
    ...
    function getCriterium($id, $data = array())
    {
        ...
        $path = $this->getEntityAndAttrPath($data['attrs']);
        $scriptCode[] = "Adapto.SmartSearchHandler.registerCriterium($id);";
        ...
        $result['remove_action'] = "Adapto.SmartSearchHandler.removeCriterium($id)";
    ...
    {
        return session_url(dispatch_url($this->m_entity->atkEntityType(), $this->m_action), SESSION_REPLACE);
    }
    ...
    function smartSearchForm($name = "", $criteria = array())
    {
        ...
        $url = session_url(dispatch_url($this->m_entity->atkEntityType(), 'smartsearch', array('atkpartial' => 'criterium'), atkSelf()), SESSION_NEW);
    ...
    function smartSearchPage($name = "", $criteria = array())
    {
        ...
        $page->register_script(Adapto_Config::getGlobal("atkroot") . "atk/javascript/class.atksmartsearchhandler.js");
        ...
        $params["formstart"] = '<form name="entryform" action="' . atkSelf() . '?' . SID . '" method="post">' . session_form(SESSION_REPLACE)
                . '<input type="hidden" name="atkaction" value="smartsearch">' . '<input type="hidden" name="atkentitytype" value="' . $entity->atkEntityType()
                . '">';

        $params["content"] = $this->invoke("smartSearchForm", $name, $criteria);

        $params["buttons"][] = '<input type="submit" class="btn_search" name="atkdosearch" value="' . atktext("search", "atk") . '">';

        $params["formend"] = '</form>';

        $action = $ui->renderAction("smartsearch", $params);
        $box = $ui->renderBox(array('title' => $entity->actionTitle('smartsearch'), 'content' => $action));
