 ...
 *
 */
class Adapto_Handler_Import extends Adapto_ActionHandler
{
    public $m_importEntity; // defaulted to public
    ...
    function action_import()
    {
        ...
        if (is_array($Adapto_VARS['allFields']))
            $keys = array_keys($Adapto_VARS['allFields']);
        foreach ($keys as $key) {
            if (!isset($Adapto_VARS[$Adapto_VARS['allFields'][$key] . "_newsel"]))
                $Adapto_VARS[$Adapto_VARS['allFields'][$key] . "_newsel"] = $Adapto_VARS[$Adapto_VARS['allFields'][$key]];
        }
    ...
    /**
     * Sets the entity for this handler. Implicitly sets the
     * import entity too!
     *
    ...
    function setEntity(&$entity)
    {
        ...
        $this->setImportEntity($entity);
    }

    /**
     * Sets the import entity. By default this is the same entity
     * as set by setEntity, but if you call this method after the setEntity
     * call you can override the import entity.
     *
     * @param atkEntity $entity entity instance
     *
     * @see setEntity
     */
    function setImportEntity(&$entity)
    {
        $this->m_importEntity = &$entity;
    }
    ...
    function importPage($phase, $content)
    {
        ...
        $formStart = '<form id="entryform" name="entryform" enctype="multipart/form-data" action="' . $action . '" method="post">'
                . session_form(atkLevel() == 0 ? SESSION_NESTED : SESSION_REPLACE) . '<input type="hidden" name="atkentitytype" value="'
                . $this->m_entity->atkEntityType() . '" />' . '<input type="hidden" name="atkaction" value="' . $this->m_entity->m_action . '" />'
        ...
        $this->m_entity->addStyle("style.css");

        $params = $this->m_entity->getDefaultActionParams(false);
        ...
        $params = array();
        $params['title'] = $this->m_entity->actionTitle('import');
        $params['content'] = $output;
        $output = $ui->renderBox($params);

        $output = $this->m_entity->renderActionPage('import', $output);
    ...
    function getImportButtons($phase)
    {
        ...
        if ($phase == 'init') {
            $result[] = '<input class="btn" type="submit" value="' . $this->m_entity->text("import_upload") . '">';
        } else if ($phase == 'analyze') {
            $result[] = '<input type="submit" class="btn" name="analyse" value="' . $this->m_entity->text("import_analyse") . '">';
            $result[] = '<input type="submit" class="btn" name="import" value="' . $this->m_entity->text("import_import") . '"> ';
        }

        if (atkLevel() > 0) {
            $result[] = atkButton($this->m_entity->text("cancel", "atk"), "", SESSION_BACK, true);
        }
        ...
        <table border="0">
          <tr>
            <td style="text-align: left">
              ' . $this->m_entity->text("import_upload_explanation")
                . '
              <br /><br />
    ...
    function doUpload()
    {
        ...
        if (!move_uploaded_file($_FILES['csvfile']['tmp_name'], $filename)) {
            $this->m_entity->redirect($this->m_entity->feedbackUrl("import", ACTION_FAILED));
        } else {
            // file uploaded
    ...
    function doAnalyze($fileid, $importerrors = array())
    {
        ...
        // Display sample
        $sample = atktext("import_sample") . ':<br><br><table class="recordlist">' . $this->_getAnalyseSample($columncount, $col_map, $csv_data, $skipfirstrow);
    ...
    function _getErrors($importerrors)
    {
        if (is_array($importerrors)) {
            ...
            foreach ($importerrors as $record => $errors) {
                ...
                if ($errorCount > Adapto_Config::getGlobal("showmaximporterrors", 50))
                    break;

                if ($record == 0 && Adapto_value_in_array($errors)) {
                    ...
                    $content .= "</td></tr>";
                } else if (Adapto_value_in_array($errors)) {
                    $content .= "<tr><td valign=\"top\" class=\"error\">";
                    $content .= "<b>Record $record:</b>&nbsp;";
                    $content .= "</td><td valign=\"top\" class=\"error\">";
                    $counter = 0;
                    for ($counter = 0; $counter < count($errors) && $counter < Adapto_Config::getGlobal("showmaximporterrors", 50); $counter++) {
                        $content .= $this->m_entity->text($errors[$counter]['msg']) . $errors[$counter]['spec'] . "<br />";
                    }
                    $content .= "</td></tr>";
                }
            }
    ...
    function _getAnalyseHeader($fileid, $columncount, $delimiter, $enclosure, $rowcount)
    {
        ...
        $content .= '<tr><td>' . text("delimiter") . ': </td><td><input type="text" size="2" name="delimiter" value="' . Adapto_htmlentities($delimiter)
                . '"></td></tr>';
        $content .= '<tr><td>' . text("enclosure") . ': </td><td><input type="text" size="2" name="enclosure" value="' . Adapto_htmlentities($enclosure)
                . '"></td></tr>';
    ...
    function _getSampleValue($attributename, $value)
    {
        ...
        if (method_exists($attr, "createDestination")) {
            ...
            if (count($searchresults) == 1) {
                $atkval = array(
                        $attributename => array($attr->m_destInstance->primaryKeyField() => $searchresults[0][$attr->m_destInstance->primaryKeyField()]));
            }
        } else {
    ...
    function _getAnalyseExtraOptions($skipfirstrow, $doupdate, $updatekey1, $onfalseidentifier, $allFields, $novalidatefirst)
    {
        ...
        $content .= atktext("import_skipfirstrow") . ': </td><td><input type="checkbox" name="skipfirstrow" class="atkcheckbox" value="1" '
                . ($skipfirstrow ? "CHECKED" : "") . '/>';
        $content .= '</td></tr><tr><td>';
        $content .= atktext("import_doupdate") . ': </td><td> <input type="checkbox" name="doupdate" class="atkcheckbox" value="1" '
                . ($doupdate ? "CHECKED" : "") . '/>';
        $content .= '</td></tr><tr><td>';
        $content .= atktext("import_update_key") . ': </td><td>' . $this->getAttributeSelector(0, $updatekey1, "updatekey1", 2) . '</td>';
        $content .= '</td></tr><tr><td>';
        $content .= atktext("import_onfalseidentifier") . ': </td><td> <input type="checkbox" name="onfalseid" class="atkcheckbox" value="1" '
                . ($onfalseidentifier ? "CHECKED" : "") . '/>';
        $content .= '</td></tr><tr><td>';
        $content .= atktext("import_novalidatefirst") . ': </td><td> <input type="checkbox" name="novalidatefirst" class="atkcheckbox" value="1" '
                . ($novalidatefirst ? "CHECKED" : "") . '/>';
    ...
    {
        return Adapto_Config::getGlobal("atktempdir") . "csv_import_$fileid.csv";
    }
    ...
    {
        $attrs = array();
        foreach (array_keys($this->m_importEntity->m_attribList) as $attribname) {
            $attrib = &$this->m_importEntity->getAttribute($attribname);
    ...
    {
        return ((!$obligatoryOnly || $this->isObligatory($attrib)) && !$attrib->hasFlag(AF_AUTOINCREMENT) && !$this->isHide($attrib)
    ...
    /**
     * Get al attributes from the import entity that have the flag AF_ONETOONE_INTEGRATE
     * @return array  A list with all attributes from the import entity that have the flag AF_ONETOONE_INTEGRATE
     */
    ...
    {
        if (array_key_exists($attrname, $this->m_importEntity->m_attribList))
            return false;

        foreach ($this->getIntegratedAttributes() as $attr) {
            $relattr = $this->m_importEntity->getAttribute($attr);
    ...
    {
        if (array_key_exists($name, $this->m_importEntity->m_attribList))
            return $this->m_importEntity->getAttribute($name);
    ...
    /**
     * Returns a dropdownlist with all possible field in the importentity
    ...
    function getAttributeSelector($index = 0, $value = "", $othername = "", $emptycol = 2)
    {
        ...
        if ($emptycol == 2)
            $res .= '<option value="-" ' . (($value == "-" || !$hasoneselected) ? "selected" : "") . ' style="font-style: italic">'
                    . atktext("import_ignorecolumn");
        elseif ($emptycol == 1)
    ...
    function inArray($array, $value)
    {
        foreach ($array as $key => $item) {
            ...
            if (strtolower($item) == strtolower(atktext($value, $this->m_entity->m_module, $this->m_entity->m_type)))
                return $key;
        }
        return false;
    }
        ...
        foreach ($allFields as $key => $allField) {
            if ($allField != "") {
                ...
                if (strstr($allFieldValue, "=")) {
                    ...
                    $allFieldValue = substr($allFieldValue, 0, Adapto_strlen($allFieldValue) - 1);
                }
    ...
    function doImport($nopost = false)
    {
        ...
        if ($count == 0) {
            $messageQueue->addMessage(sprintf($this->m_entity->text('no_records_to_import'), $count), AMQ_GENERAL);
        } else if ($count == 1) {
            $messageQueue->addMessage($this->m_entity->text('successfully_imported_one_record'), AMQ_SUCCESS);
        } else {
            $messageQueue->addMessage(sprintf($this->m_entity->text('successfully_imported_x_records'), $count), AMQ_SUCCESS);
        }
    ...
    function getValidatedRecords($file)
    {
        ...
        $initial_values = $this->m_importEntity->initial_values();
                    ...
                    } else //this is the allField
 {
    ...
    function _getAttributeValue($attributename, $allFields, $value, &$importerrors, $counter, $rec)
    {
        ...
        if (method_exists($attr, "createDestination") && $attr->createDestination() && !in_array($attributename, $allFields)) {
            ...
            if (!$isNumeric || is_numeric($value)) {
                $relationselect = $attr->m_destInstance
                        ->selectDb($attr->m_destInstance->m_table . "." . $attr->m_destInstance->primaryKeyField() . ' = \'' . escapeSQL($value) . "'");
            }
            ...
            if (count($relationselect) == 0 || count($relationselect) > 1) {
                ...
                if (count($searchresults[$attributename][$value]) == 1) {
                    $value = array(
                            $attr->m_destInstance->primaryKeyField() => $searchresults[$attributename][$value][0][$attr->m_destInstance->primaryKeyField()]);
                } else {
            ...
            foreach ($validrecs as $validrec) {
                ...
                if (!empty($validrec['atkerror'])) {
                    foreach ($validrec['atkerror'] as $atkerror) {
                        $importerrors[$counter][] = array("msg" => "Fouten gedetecteerd op rij $counter: ", "spec" => $atkerror['msg']);
                    }
                }
                unset($validrec);
            }
    ...
    function add(&$record)
    {
        $this->m_importEntity->preAdd($record);

        if (isset($record['atkerror'])) {
            return false;
        }

        $this->m_importEntity->addDb($record);
    ...
    {
        $this->m_importEntity->preUpdate($record);

        if (isset($record['atkerror'])) {
            return false;
        }

        $this->m_importEntity->updateDb($record);
    ...
    function validate(&$record)
    {
        ...
        $this->m_importEntity->validate($record, $mode);
     ...
     * @param array $initial_values The initial_values of the importentity
    ...
    function checkImport($col_map, $initial_values)
    {
        ...
        $this
                ->_returnErrors(array_values(array_diff($unused, array_keys($initial_values))), "import_error_fieldisobligatory",
        ...
        foreach ($fields as $field) {
            ...
            if (!$result)
                if (in_array($field, $this->getObligatoryAttributes())) {
                    return array(
                            'msg' => sprintf(atktext("import_error_allfieldnocorrectdata"), atktext($field, $this->m_entity->m_module, $this->m_entity->m_type),
                                    public_export($values[$field], 1))); // defaulted to public
                } else {
                    $value = "";
                }
        }
        ...
        if (!$this->validate($rec)) {
            ...
            if ($errors[0]) {
                foreach ($errors as $error) {
                    ...
                    $importerrors[$counter][] = array("msg" => $error['msg'] . ": ", "spec" => $attr->label());
                }
            }
        }
    ...
    function prepareUpdateRecord(&$record)
    {
        ...
        $this->m_importEntity->m_postvars["atksearchmode"] = "exact";
        //      if (!in_array($allFieldValue)) $this->m_importEntity->m_fuzzyFilters[] = $allFieldValue;

        $dbrec = $this->m_importEntity->searchDb(array($updatekey1 => $updatekey1val));

        if (count($dbrec) == 1) {
            $record[$this->m_importEntity->primaryKeyField()] = $dbrec[0][$this->m_importEntity->primaryKeyField()];
