 ...
 * @subpackage menu
 */
class Adapto_Menu_Outlook extends Adapto_menuinterface
{
    /**
     * Constructor
     *
     * @return Adapto_Menu_Outlook
     */
    ...
    function render()
    {
    	...
    	");
            ...
            $page->register_script(Adapto_Config::getGlobal("atkroot") . "atk/javascript/crossbrowser.js");
            $page->register_script(Adapto_Config::getGlobal("atkroot") . "atk/javascript/outlook.js");
            ...
            while (list($name) = each($g_menu)) {
                ...
                for ($i = 0; $i < count($g_menu[$atkmenutop]); $i++) {
                    ...
                    /* submenu ? */
                        ...
                        $menu .= href('menu.php?atkmenutop=' . $name, $this->getMenuTranslation($name, $modname), SESSION_DEFAULT)
                                . Adapto_Config::getGlobal("menu_delimiter");
                    } else if (empty($url) && !$enable) {
                    ...
                    /* normal menu item */
                        ...
                        if ($g_menu[$atkmenutop][$i]["url"] != "") {
                            ...
                            if (file_exists($menu_icon)) {
                                $panelbuttons .= "p.addButton('" . $menu_icon . "','" . addslashes($this->getMenuTranslation($name, $modname))
                                        . "','parent.main.location.href = \"" . "" . $url . "\"');\n";
                            } else {
                                $panelbuttons .= "p.addButton('" . $theme->imgPath('empty.gif') . "','" . addslashes($this->getMenuTranslation($name, $modname))
                                        . "','parent.main.location.href = \"" . "" . $url . "\"');\n";
                            }
                        }
                    } else {
