...
/**
 * The Adapto_Attribute_MlText class represents an multilanguage attribute
 * of an entity that is a big text field.
 *
 ...
 *
 */ 
class Adapto_Attribute_MlText extends Adapto_MlAttribute
{
    ...
    function edit($record = "", $prefix = "", $mode = "")
    {
        ...
        for ($i = 1; $i < sizeof($languages); $i++) {
            $result .= '<input type="hidden" name="' . $prefix . $this->formName() . '[' . $languages[$i] . ']"' . ' value="'
                    . htmlspecialchars($record[$this->fieldName()][$languages[$i]]) . '">';
        }

        /* default language */
        $result .= '<textarea name="' . $prefix . $this->formName() . '[' . $languages[0] . ']"' . ' rows="10" wrap="virtual"'
                . ($this->m_size > 0 ? ' cols="' . $this->m_size . '"' : '') . '>' . htmlspecialchars($record[$this->fieldName()][$languages[0]])
                . '</textarea>';
        ...
        if ($this->m_edited) // if we edited this attrib, we also need to add the
 {
            ...
            $entry["html"] = '<textarea name="' . $fieldprefix . $this->formName() . '[' . (count($this->m_languages) == 2 ? $curlng : "multilanguage") . ']"'
                    ...
                    . htmlspecialchars($defaults[$this->fieldName()][$curlng]) . '</textarea>';
