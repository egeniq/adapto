...
/**
 * The Adapto_Attribute_DateTime class can be used for date and time entry.
 ...
 *
 */
class Adapto_Attribute_DateTime extends Adapto_Attribute
{
    public $m_time = ""; // defaulted to public
    public $m_date = ""; // defaulted to public

    public $m_utcOffset = null; // defaulted to public
    public $m_timezoneAttribute = null; // defaulted to public
    ...
    public function __construct($name, $default_date = "", $default_time = "", $flags = 0)
    {
        ...
        $this->m_time = new Adapto_Timeattribute($name, 0, 23, $default_steps, $default_time, $flags);
        $this->m_date = new Adapto_Dateattribute($name, '', '', 0, 0, $flags);

        parent::__construct($name, $flags); // base class constructor

        if ($this->hasFlag(AF_OBLIGATORY)) {
            $this->setInitialValue(Adapto_Attribute_DateTime::datetimeArray());
        }
    }
    ...
    function addToQuery(&$query, $tablename = "", $fieldaliasprefix = "", $rec = "", $level, $mode)
    {
        if ($mode == "add" || $mode == "update") {
            ...
            if (Adapto_Config::getGlobal('database') != 'oci9')
    ...
    function _getUTCOffset(&$record, $stamp = null)
    {
        if ($this->m_utcOffset !== null) {
            ...
            $entity = $this->getOwnerInstance();

            while (count($parts) > 0) {
                $part = array_shift($parts);
                $attr = $entity->getAttribute($part);
    ...
    function postInit()
    {
        if ($this->m_timezoneAttribute !== null) {
            $entity = &$this->getOwnerInstance();
            $parts = explode('.', $this->m_timezoneAttribute);
            $attr = &$entity->getAttribute($parts[0]);
