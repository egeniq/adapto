...
/**
 * Flag(s) specific for Adapto_Attribute_Password
 */
...
/**
 * The Adapto_Attribute_Password class represents an attribute of an entity
 ...
 *
 */
class Adapto_Attribute_Password extends Adapto_Attribute
{
    /* generate? */
    public $m_generate; // defaulted to public
     ...
     * @var Array
     */
    public $m_restrictions; // defaulted to public
    ...
    public function __construct($name, $generate, $flags = 0, $size = 0, $restrictions = "")
    {
    ...
    {
        $this->m_restrictions = array("minsize" => 0, "minupperchars" => 0, "minlowerchars" => 0, "minalphabeticchars" => 0, "minnumbers" => 0,
                "minspecialchars" => 0);
    ...
    function edit($record = "", $fieldprefix = "", $mode = "")
    {
        ...
        if ($mode != 'edit' && $mode != 'update') {
            if (!$this->m_generate) {
                ...
                $result = atktext("password_new", "atk") . ':<br>' . '<input autocomplete="off" type="password" id="' . $id . '[new]" name="' . $id . '[new]"'
                        . ($this->m_maxsize > 0 ? ' maxlength="' . $this->m_maxsize . '"' : '') . ($this->m_size > 0 ? ' size="' . $this->m_size . '"' : '')
                        . "><br><br>" . atktext("password_again", "atk") . ':<br>' . '<input autocomplete="off" type="password" id="' . $id . '[again]" name="'
                        . $id . '[again]"' . ($this->m_maxsize > 0 ? ' maxlength="' . $this->m_maxsize . '"' : '')
                        . ($this->m_size > 0 ? ' size="' . $this->m_size . '"' : '') . ">";
            } else {
                $password = $this->generatePassword(8, TRUE);
                $this->registerKeyListener($id . '[new]', KB_CTRLCURSOR | KB_UPDOWN);
                $this->registerKeyListener($id . '[again]', KB_CTRLCURSOR | KB_UPDOWN);
                $result = '<input type="hidden" id="' . $id . '[again]" name="' . $id . '[again]"' . ' value ="' . $password . '">' . '<input type="text" id="'
                        . $id . '[new]" name="' . $id . '[new]"' . ($this->m_maxsize > 0 ? ' maxlength="' . $this->m_maxsize . '"' : '')
                        . ($this->m_size > 0 ? ' size="' . $this->m_size . '"' : '') . ' value ="' . $password . '" onchange="this.form.elements[\''
                        . $fieldprefix . $this->fieldName() . '[again]\'].value=this.value">';
            }
        } /* edit */
 else {
            ...
            if (!$this->hasFlag(AF_PASSWORD_NOVALIDATE)) {
                ...
                $result .= atktext("password_current", "atk") . ':<br>' . '<input autocomplete="off" type="password" id="' . $id . '[current]" name="' . $id
                        . '[current]"' . ($this->m_maxsize > 0 ? ' maxlength="' . $this->m_maxsize . '"' : '')
    ...
    function validate(&$record, $mode)
    {
        ...
        if ($mode == 'update' && (Adapto_strlen($value["new"]) > 0 || Adapto_strlen($value["again"]) > 0) && !$this->hasFlag(AF_PASSWORD_NOVALIDATE)
        ...
        if ($mode == "add" && $this->hasFlag(AF_OBLIGATORY) && Adapto_strlen($value["new"]) == 0) {
            $error = TRUE;
        ...
        if ((Adapto_strlen($value["new"]) > 0) && (!$this->validateRestrictions($value["newplain"]))) {
            $error = TRUE;
            triggerError($record, $this->fieldName(), $this->getRestrictionsText());
        }

        // new password?
        if (!$error && Adapto_strlen($value["new"]) > 0)
