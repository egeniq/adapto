...
function drawTD($field, $color, $colHeight, $colWidth, $extern = 0)
{
    ...
    $row .= "('" . $field . "','" . $color . "')";
    $row .= '" title="' . atktext("color", "atk") . ': ' . $color . '"><IMG SRC="' . Adapto_Config::getGlobal('atkroot') . 'atk/images/dummy.gif" border=0 width=' . $colWidth
            . ' height=' . $colHeight . ' alt="-X-"></a></td>';
    $row .= "\n";
...
/**
 * The Adapto_Attribute_ColorPicker class represents an attribute of an atkEntity.
 * An Adapto_Attribute_ColorPicker shows a box with 10 user defined colors and 90 pre-defined colors
 ...
 *
 */
class Adapto_Attribute_ColorPicker extends Adapto_Attribute
{
    public $m_userColors; // defaulted to public
    public $m_currentColor; // defaulted to public

    public $m_colorMode = CP_COLORMODE_DEFAULT; // defaulted to public

    /**
     * Constructor
     *
     * <b>Example:</b>
     * $this->add(new Adapto_Attribute_ColorPicker("naam",Array,AF_OBLIGATORY|AF_POPUP));
     * @param string $name Name of the attribute
     * @param array $userColors Array with max. 12 user defined colors
     * @param int $flags Flags for the attribute
     */

    public function __construct($name, $userColors = "", $flags = 0)
    {
    ...
    function edit($record = "", $fieldprefix = "", $mode = "")
    {
        ...
        $page->register_script(Adapto_Config::getGlobal("atkroot") . "atk/javascript/colorpicker.js");
        ...
        $colorField = '<input type="hidden" name="' . $fieldprefix . $this->formName() . '" id="' . $fieldprefix . $this->formName() . '" value="'
                . $this->m_currentColor . '" size="7" maxlength="7">';
        ...
        if (trim($this->m_currentColor) == "") {
            $selectImg = Adapto_Config::getGlobal("atkroot") . "atk/images/select_color_off.gif";
            $alt = atkText("no_color_selected", "atk");
        } else {
            $selectImg = Adapto_Config::getGlobal("atkroot") . "atk/images/select_color_on.gif";
        ...
        $url = atkPopup('atk/popups/colorpicker.inc', 'field=' . $fieldprefix . $this->fieldName() . '&activecolor=' . $urlColor . '&usercol=' . $userCol,
                'colorPicker', 315, 1000, 'no', 'no');
        $text = '<a href="javascript:void(0);" onclick="' . $url . '; return false;"><img name="img_' . $fieldprefix . $this->fieldName() . '" id="img_'
                . $fieldprefix . $this->fieldName() . '" src="' . $selectImg . '" border="0" alt="' . $alt . '" style="background-color: ' . $alt . ';"></a>'
                . $colorField;
        $result = '<table cellpadding="0" cellspacing="0" border="0" width="35" height="21" align="left"><tr><td id="example_' . $fieldprefix
                . $this->fieldName() . '" name="example_' . $fieldprefix . $this->fieldName() . '" style="padding:0px;" valign="top" align="left" bgcolor="'
                . $this->m_currentColor . '">' . $text . '</td></tr></table>';
    ...
    function checkTypeAndMode($type)
    {
        ...
        if (($type == 'hex') && in_array($this->getColorMode(), array(CP_COLORMODE_DEFAULT, CP_COLORMODE_HEX6))) {
            return true;
        }

        // For rgb types, the colormode must be default.
        if (($type == 'rgb') && ($this->getColorMode() == CP_COLORMODE_DEFAULT)) {
            return true;
        }
