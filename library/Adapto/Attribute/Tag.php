...
/**
 * Flags for use in Adapto_Attribute_Tag constructor
 */
...
/**
 * This attribute is used for adding tags to an entity
 * For example: This attribute can be used in a blog entity
 ...
 * @subpackage attributes
 */
class Adapto_Attribute_Tag extends Adapto_FuzzySearchAttribute
{
    public $m_link = ""; // defaulted to public
    public $m_linkInstance = NULL; // defaulted to public
    public $m_destination = ""; // defaulted to public
    public $m_destInstance = NULL; // defaulted to public
    public $m_destinationfield = ""; // defaulted to public
    public $m_remoteKey = ""; // defaulted to public
    public $m_localKey = ""; // defaulted to public
    public $m_nonematching = array(); // defaulted to public
     ...
     * @param int $flags
     * @param int $size
     * @return Adapto_Attribute_Tag
     */

    public function __construct($name, $destination, $destinationfield, $link, $mode = TA_ADD, $flags = 0, $size = 0)
    {
        if ($size == 0) {
    ...
    function createDestinationInstance()
    {
        if (!is_object($this->m_destInstance)) {
            $this->m_destInstance = &getEntity($this->m_destination);
    ...
    function createLink()
    {
        if (!is_object($this->m_linkInstance)) {
            $this->m_linkInstance = &getEntity($this->m_link);
    ...
    function isValidKeyWord($keyword)
    {
        ...
        return (Adapto_strlen($keyword) == Adapto_strlen($replaced));
    }
    ...
    function store($db, $rec, $mode)
    {
        ...
        foreach ($resultset as $res) {
            ...
            $where = $this->m_linkInstance->m_table . '.' . $this->getLocalKey() . "='" . $rec[$this->m_ownerInstance->primaryKeyField()] . "'" . " AND "
                    . $this->m_linkInstance->m_table . '.' . $this->getRemoteKey() . "='" . $remKey . "'";
