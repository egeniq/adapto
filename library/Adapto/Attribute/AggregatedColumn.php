...
/**
 * The Adapto_Attribute_AggregatedColumn aggregates multiple attributes to one colunm in
 ...
 *
 */
class Adapto_Attribute_AggregatedColumn extends Adapto_Attribute
{
    /**
     ...
     * @access private
     */
    public $m_template; // defaulted to public
     ...
     * @access private
     */
    public $m_searchfields = array(); // defaulted to public
     ...
     * @access private
     */
    public $m_displayfields = array(); // defaulted to public
    ...
    public function __construct($name, $template, $flags = 0, $searchfields = "")
    {
        parent::__construct($name, $flags | AF_HIDE_EDIT | AF_HIDE_ADD | AF_HIDE_VIEW); // base class constructor
        $this->m_template = $template;

        $parser = new Adapto_StringParser($template);
    ...
    function display($record, $mode = "")
    {
        ...
        $parser = new Adapto_StringParser($this->m_template);
    ...
    function addToListArrayHeader($action, &$arr, $fieldprefix, $flags, $atksearch, $atkorderby)
    {
        if (!$this->hasFlag(AF_HIDE_LIST) && !($this->hasFlag(AF_HIDE_SELECT) && $action == "select")) {
            ...
            if (!hasFlag($flags, RL_NO_SORT) && !$this->hasFlag(AF_NO_SORT)) {
                ...
                $arr["heading"][$fieldprefix . $this->fieldName()]["url"] = session_url(
                        atkSelf() . '?atkentitytype=' . $this->m_ownerInstance->atkEntityType() . '&atkaction=' . $action . '&atkorderby=' . rawurlencode($order));
            }
            ...
            if (!hasFlag($flags, RL_NO_SEARCH) && $this->hasFlag(AF_SEARCHABLE)) {
                ...
                $arr["search"][$fieldprefix . $this->fieldName()] .= '<input type="hidden" name="atksearchmode[' . $this->formName() . ']" value="'
                        . $this->getSearchMode(false) . '">';
            }
        }
    }
    ...
    function addToQuery(&$query, $tablename = "", $fieldaliasprefix = "", $rec = "", $level, $mode)
    {
        if ($mode !== 'add' && $mode != 'edit') {
            $allfields = Adapto_array_merge($this->m_displayfields, $this->m_searchfields);
    ...
    function getSearchCondition(&$query, $table, $value, $searchmode)
    {
        ...
        if ($searchmode == 'substring') {
            ...
            // we remove all these separators (defined in the entity with new Adapto_Attribute_AggregatedColumn)
