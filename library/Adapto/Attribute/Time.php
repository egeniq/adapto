...
/**
 * Flags for Adapto_Attribute_Time
 */
define("AF_TIME_SECONDS", AF_SPECIFIC_1);

/**
 * The Adapto_Attribute_Time class represents an attribute of an entity
 ...
 *
 */
class Adapto_Attribute_Time extends Adapto_Attribute
{
    public $m_beginTime = 0; // defaulted to public
    public $m_endTime = 23; // defaulted to public
    public $m_steps = array("0", "30"); // defaulted to public
    public $m_default = ""; // defaulted to public
    ...
    public function __construct($name, $beginTime = 0, $endTime = 23, $steps = array("00", "15", "30", "45"), $default = "", $flags = 0)
    {
    ...
    {
        return array("hours" => substr($time, 0, 2), "minutes" => substr($time, 2, 2), "seconds" => substr($time, 4, 2));
    }
    ...
    function fetchValue($postvars)
    {
        ...
        if (!is_array($result)) {
            ...
            $result = array('hours' => $result['hours'], 'minutes' => $result['minutes'], 'seconds' => $result['seconds']);
        }
    ...
    {
        if (($this->m_default == "NOW" && $this->m_ownerInstance->m_action == "add") || ($this->m_default == "" && $this->hasFlag(AF_OBLIGATORY))) {
    ...
    function db2value($rec)
    {
        if (strlen($rec[$this->fieldName()]) == 0) {
            ...
            $retval = array("hours" => substr($rec[$this->fieldName()], 0, 2), "minutes" => substr($rec[$this->fieldName()], 3, 2),
    ...
    {
        //Assuming hh:mm:ss
        $retval = array("hours" => substr($stringvalue, 0, 2), "minutes" => substr($stringvalue, 3, 2), "seconds" => substr($stringvalue, 6, 2));
