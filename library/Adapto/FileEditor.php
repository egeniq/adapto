 ...
 * File editing entity.
 *
 * This is a special derivative atkEntity that does not have database
 ...
 *
 */
class Adapto_FileEditor extends Adapto_Entity
{
    public $m_dir; // defaulted to public
    public $m_basedir; // defaulted to public
    public $m_filefilter; // defaulted to public
    public $m_showdirs; // defaulted to public

    /**
     * Constructor
     *
     * This function is called when a new Adapto_FileEditor is instantiated.
     *
     ...
     * @param int $flags The entity flags. See atkEntity for a list of possible
     *                   flags.
     */

    public function __construct($name, $dir = "", $filter = "", $flags = 0)
    {
        parent::__construct($name, $flags | EF_ADD_LINK);
        $this->m_dir = $dir;
        $this->m_basedir = $dir;
        if ($dir == "")
            $this->m_dir = "./";
        ...
        $this->add(new Adapto_Attribute("filename", AF_PRIMARY | AF_SEARCHABLE));
        $this->add(new Adapto_TextAttribute("filecontent", 30, AF_HIDE_LIST));
    ...
    /**
     * This function is used to change the entity directory
     *
    ...
    /**
     * This function adds a file or directory to the list in the entity
     *
    ...
    function recordActions($record, &$actions, &$mraactions)
    {
        ...
        if (is_dir($this->m_dir . "/" . $record["filename"])) {
            $actions['view'] = dispatch_url(parent::__constructType(), "dirchange", array('atkselector' => $this->m_dir . $record["filename"]));
            unset($actions["edit"]);
    ...
    function selectDb($selector = "", $orderby = "", $limit = "")
    {
        ...
        if ($selector == "") {
            ...
            // atkentity uses the value in some places.
    ...
    function deleteDb($selector)
    {
        ...
        Adapto_var_dump($this->m_dir, 'm_dir');
        Adapto_var_dump($filename, 'filename');
