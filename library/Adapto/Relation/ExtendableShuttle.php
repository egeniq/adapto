 ...
 * @subpackage relations
 */
class Adapto_Relation_ExtendableShuttle extends Adapto_ManyToManyRelation
{
    ...
    public function partial_filter()
    {
        ...
        if ($redraw) {
            $res .= "$('" . $this->getHtmlId($prefix) . "_" . $record[$this->fieldName()]["section"] . "').innerHTML = "
                    . atkJSON::encode($this->renderSelectBoxes($record[$this->fieldName()]["section"], $record, $mode, $prefix)) . ";";
        }
        $res .= "</script>";
    ...
    public function edit($record, $fieldprefix = "", $mode = "")
    {
        ...
        $mode == "add" ? "add" : "edit";
        $url = addslashes(
                partial_url($this->m_ownerInstance->atkEntityType(), $mode, "attribute." . $this->getHtmlId($fieldprefix) . ".selection",
                        array("atkfieldprefix" => $fieldprefix)));
        $this
                ->addOnChangeHandler(
                        "shuttle_refresh('$url', '" . $this->getHtmlId($fieldprefix) . '[cselected][][' . $this->getRemoteKey() . ']' . "', '" . $prefix
                                . $this->fieldName() . "[section]', el);");
        ...
        $page->register_script(Adapto_Config::getGlobal("atkroot") . "atk/javascript/class.atkextendableshuttlerelation.js");
        $page->register_submitscript("shuttle_selectAll('" . $rightname . "');");

        $ui = ATKUI::getInstance();
        $result = $ui
                ->render('extendableshuttle.tpl',
                        array('leftname' => $leftname, 'rightname' => $rightname, 'name' => $name, 'htmlid' => $this->getHtmlId($fieldprefix),
                                'remotekey' => $this->getRemoteKey(), 'value' => $value, 'ava_controls' => $ava_controls, 'sel_controls' => $sel_controls,
                                'available_options' => $available_options, 'selected_options' => $selected_options,));
    ...
    protected function _renderSelect($name, $recordset, $opposite, $prefix, $isSelected)
    {
        ...
        $result = '<select class="shuttle_select" id="' . $name . '" name="' . $name . '" multiple size="10" onDblClick="shuttle_move(\'' . $name . '\', \''
                . $opposite . '\',\'' . $action . '\',\'' . $valName . '\');' . $onchangecode . '">';

        $parser = null;
        // Only import the stringparser once.
        if (isset($this->m_descriptor_tooltip_template)) {

            $parser = new Adapto_StringParser($this->m_descriptor_tooltip_template);
        }
        ...
        for ($i = 0, $_i = count($recordset); $i < $_i; $i++) {
            ...
            $result .= '<option value="' . $recordset[$i][$this->m_destInstance->primaryKeyField()] . '" title="' . $ttip . '">' . Adapto_htmlentities($title)
                    . '</option>';
        }
        $result .= '</select>';
        return $result;
