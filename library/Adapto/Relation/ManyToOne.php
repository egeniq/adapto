...
class Adapto_Relation_ManyToOne extends Adapto_Relation
{
        ...
        } else // AF_LARGE || AF_RELATION_AUTOCOMPLETE
 {
    ...
    {
        if ($this->createDestination()) {
            return (($recA[$this->fieldName()][$this->m_destInstance->primaryKeyField()] == $recB[$this->fieldName()][$this->m_destInstance->primaryKeyField()])
    ...
    function dbFieldType()
    {
        ...
        // the type of field of the primary key of the entity we have a
        // relationship with.
        if ($this->createDestination()) {
    ...
    function dbFieldSize()
    {
        ...
        // the size of the field of the primary key of the entity we have a
        // relationship with.
        if ($this->createDestination()) {
            if (count($this->m_refKey) > 1) {
                $keys = array();
    ...
    function createFilter($record)
    {
        if ($this->m_destinationFilter != "") {
            ...
            $parser = new Adapto_StringParser($this->m_destinationFilter);
    ...
    function getSelectableRecords($record = array(), $mode = "select")
    {
        ...
        $result = $this->m_destInstance->select($selector)->orderBy($this->getDestination()->getOrder())
                ->includes(Adapto_array_merge($this->m_destInstance->descriptorFields(), $this->m_destInstance->m_primaryKey))->mode($mode)->allRows();
    ...
    function getJoinCondition(&$query, $tablename = "", $fieldalias = "")
    {
        ...
        if ($this->m_joinFilter != "") {
            ...
            $parser = new Adapto_StringParser($this->m_joinFilter);
    ...
    function getOrderByStatement($extra = '', $table = '', $direction = 'ASC')
    {
        ...
        if (!empty($order)) {
            ...
            foreach ($parts as $part) {
                ...
                // in the destination entity even if the default is descending
    ...
    protected function _addColumnToListArrayHeader($column, $action, &$arr, $fieldprefix, $flags, $atksearch, $atkorderby, atkDataGrid $grid = null)
    {
        ...
        if ($p_attrib == null) {
            throw new Exception(
                    "Invalid list column {$column} for Adapto_Relation_ManyToOne " . $this->getOwnerInstance()->atkEntityType() . '::' . $this->fieldName());
        }
        ...
        foreach (array_keys($arr['heading']) as $key) {
            ...
            if (is_object($atkorderby) && isset($atkorderby->m_colcfg[$this->fieldName()]) && isset($atkorderby->m_colcfg[$this->fieldName()]['extra'])
     ...
     */
    function addToSearchformFields(&$fields, &$entity, &$record, $fieldprefix = "")
    {
        $prefix = $fieldprefix . $this->fieldName() . "_AE_";

        parent::addToSearchformFields($fields, $entity, $record, $prefix);
        ...
        foreach ($this->m_listColumns as $attribname) {
            ...
            if (!$p_attrib->hasFlag(AF_HIDE_SEARCH)) {
                $p_attrib->addToSearchformFields($fields, $entity, $record[$this->fieldName()], $prefix);
            }
        }
    }
    ...
    function drawAutoCompleteBox($record, $fieldPrefix, $mode)
    {
        ...
        $result = '<input type="hidden" id="' . $id . '" name="' . $id . '" value="' . $value . '" />
         <input type="text" id="' . $id . '_search" value="' . atk_htmlentities($label) . '" class="atkmanytoonerelation_search" size="'
                . $this->m_autocomplete_size . '" onfocus="this.select()" />
        ...
        $url = partial_url($this->m_ownerInstance->atkEntityType(), $mode, 'attribute.' . $this->fieldName() . '.autocomplete');
    ...
    function partial_autocomplete($mode)
    {
        ...
        if (Adapto_strlen($searchvalue) < $this->m_autocomplete_minchars) {
        ...
        foreach ($records as $rec) {
            $option = Adapto_htmlentities(strip_tags($this->m_destInstance->descriptor($rec)));
    ...
    function partial_autocomplete_search()
    {
        ...
        foreach ($records as $rec) {
            ...
            $value = $this->m_destInstance->primaryKey($rec);
            $result .= '
          <li title="' . Adapto_htmlentities($option) . '">' . Adapto_htmlentities($option) . '</li>';
        }
