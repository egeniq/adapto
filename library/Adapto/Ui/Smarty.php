 ...
 *
 */
class Adapto_Ui_Smarty
{

    /**
     * Get the Smarty  instance.
     *
     * Adapto_Ui_Smarty is a singleton.
     *
     * @static
    ...
    function &getInstance()
    {
        ...
        if ($s_smarty == NULL) {
            ...
            $s_smarty->template_dir = Adapto_Config::getGlobal("tplroot"); // name of directory for templates
            ...
            $s_smarty->force_compile = Adapto_Config::getGlobal("smarty_forcecompile"); // force templates to compile every time,
            // overrides cache settings. default false.

            $s_smarty->caching = Adapto_Config::getGlobal("tplcaching"); // enable caching. can be one of 0/1/2.
            // 0 = no caching
            // 1 = use class cache_lifetime value
            // 2 = use cache_lifetime in cache file
            // default = 0.
            $s_smarty->cache_dir = Adapto_Config::getGlobal("tplcachedir"); // name of directory for template cache files
            $s_smarty->use_sub_dirs = Adapto_Config::getGlobal("tplusesubdirs"); // use subdirs for compiled and cached templates
            $s_smarty->cache_lifetime = Adapto_Config::getGlobal("tplcachelifetime"); // number of seconds cached content will persist.
            ...
            $s_smarty->default_handler = Adapto_Config::getGlobal("defaulthandler");
            $s_smarty->default_modifier = Adapto_Config::getGlobal("defaultmodifier");

            // plugin dirs
            $s_smarty->plugins_dir = array(Adapto_Config::getGlobal("atkroot") . "atk/ui/smarty/plugins", Adapto_Config::getGlobal("atkroot") . "atk/ui/plugins");
            $customplugindir = Adapto_Config::getGlobal("tplplugindir");
            if ($customplugindir != "")
    ...
    function addFunction($moduleOrPath, $tag, $name = "", $cacheable = true, $cache_attrs = null)
    {
        ...
        $path = Adapto_Ui_Smarty::getPathForPlugin($moduleOrPath, $name, 'function');
    ...
    {
        Adapto_Ui_Smarty::addFunction($moduleOrPath, $tag, $tag, false, $cache_attrs);
    }
    ...
    function addCompilerFunction($moduleOrPath, $tag, $name = "", $cacheable = true)
    {
        ...
        $path = Adapto_Ui_Smarty::getPathForPlugin($moduleOrPath, $name, 'compiler');
    ...
    function addBlock($moduleOrPath, $tag, $name = "", $cacheable = true)
    {
        ...
        $path = Adapto_Ui_Smarty::getPathForPlugin($moduleOrPath, $name, 'block');
    ...
    {
        Adapto_Ui_Smarty::addBlock($moduleOrPath, $tag, $tag, false);
    }
    ...
    function addModifier($moduleOrPath, $tag, $name = "")
    {
        ...
        $path = Adapto_Ui_Smarty::getPathForPlugin($moduleOrPath, $name, 'modifier');
 ...
 */
Adapto_Ui_Smarty::addDynamicFunction('atk.ui.plugins', 'atkfrontcontroller');
