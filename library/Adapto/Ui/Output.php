 ...
 *
 * Any output sent to the browser should be directed through the Adapto_Ui_Output
 ...
 *
 * The Adapto_Ui_Output class is a singleton. The one-and-only instance should be
 ...
 *
 */
class Adapto_Ui_Output
{
    /**
     * Store raw output data.
     * @access private
     */
    public $m_raw = ""; // defaulted to public
    ...
    /**
     * Retrieve the one-and-only Adapto_Ui_Output instance.
     * @return Adapto_Ui_Output The instance.
     */
    ...
    public function sendCachingHeaders($lastmodificationstamp = '', $nocache = true)
    {
        ...
        if ($nocache) {
            Adapto_Ui_Output::sendNoCacheHeaders();
            ...
            if ($_last_modified_date == $_gmt_mtime) {
                Adapto_Ui_Output::header("HTTP/1.0 304 Not Modified");
                return;
            } else {
                Adapto_Ui_Output::header("Last-Modified: " . gmdate("D, d M Y H:i:s", $lastmodificationstamp) . " GMT");
            }
        }
    }
    ...
    public static function sendNoCacheHeaders()
    {
        ...
        Adapto_Ui_Output::header("Cache-Control: post-check=0, pre-check=0", false);
        Adapto_Ui_Output::header("Pragma: no-cache"); // HTTP/1.0
    }
    ...
    public function outputFlush($nocache = true, $lastmodificationstamp = "", $charset = "")
    {
        ...
        if (strlen($this->m_raw) > 0) {
            ...
            Adapto_Ui_Output::header("Content-Type: text/html; charset=" . ($charset == "" ? atkGetCharset() : $charset));
        ...
        if (Adapto_Config::getGlobal("output_gzip") && phpversion() >= '4.0.4pl1'
            ...
            Adapto_Ui_Output::header('Content-Encoding: gzip');
            echo $this->gzip($res);
        } else {
            echo $res;
        }
    ...
    public function getDebugging()
    {
        ...
        if (Adapto_Config::getGlobal("debug") > 0) {
            $output = '<br><div style="font-family: monospace; font-size: 11px;" align="left" id="Adapto_debugging_div">' . implode("<br>\n ", $g_debug_msg)
                    . '</div>';
            return $output;
        }
        return "";
     ...
     *
     * Called internally by Adapto_Ui_Output when $config_output_gzip is set to true,
