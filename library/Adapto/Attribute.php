...
class Adapto_Attribute
{
     ...
     *
     * The regular Adapto_Attribute returns false if AF_READONLY_EDIT or
     ...
     *
     * The regular Adapto_Attribute does not make use of the $record parameter;
     ...
     *
     * By default, this is the module of the owning entity of this attribute.
     ...
     * has its own store() implementation. The regular Adapto_Attribute checks if
     ...
     * has its own load() implementation. The regular Adapto_Attribute checks if a
    ...
    {
        $configsize = Adapto_Config::getGlobal("max_input_size");
     ...
     * called to retrieve the tabs. The regular Adapto_Attribute has no
     ...
     * navigation, usually from its edit() method. The regular Adapto_Attribute
    ...
    {
        return ($this->getTabs() == "*" || Adapto_in_array($tab, $this->getTabs()));
    }
    ...
    function addToSearchformFields(&$fields, &$entity, &$record, $fieldprefix = "", $currentSearchMode = array())
    {
        ...
        if (method_exists($entity, $funcname)) {
            $field["widget"] = $entity->$funcname($defaults, true, $fieldprefix);
        } else {
    ...
    function sortOptions(&$columnConfig, $fieldprefix = '', $grid = null)
    {
        if (!$this->hasFlag(AF_TOTAL) && $columnConfig->totalizable()) {
            ...
            if ($grid == null) {
                ...
                $res = '<a href="javascript:void(0)" onclick="' . Adapto_htmlentities($call) . '">' . $this->text("column_" . $cmd) . '</a>';
            }
        }
        return "";
    }
    ...
    function sortOrder(&$columnConfig, $fieldprefix = '', $grid = null)
    {
        ...
        if ($currentOrder > 0) {
            ...
            if ($grid == null) {
                ...
                $res = '<a href="javascript:void(0)" onclick="' . Adapto_htmlentities($call) . '">' . $this->text("column_" . $direction) . '</a>';
            }
        }

        $res .= '<select name="atkcolcmd[][setorder][' . $fieldprefix . $fieldname . ']">';
        $res .= '<option value="">';
        for ($i = 1; $i < 6; $i++) {
    ...
    /**
     * Retrieves the ORDER BY statement for this attribute's entity.
     ...
     *                     statement
     * @param String $table The table name (if not given uses the owner entity's table name)
    ...
    function getOrderByStatement($extra = '', $table = '', $direction = 'ASC')
    {
        ...
        if (strpos($table, '.') !== false) {
            ...
            return "LOWER(" . $this->getDb()->quoteIdentifier($table) . "." . $this->getDb()->quoteIdentifier($this->fieldName()) . ")"
                    . ($direction ? " {$direction}" : "");
        }
    ...
    function partial_refresh($mode)
    {
        ...
        foreach ($arr['fields'] as $field) {
            ...
            $value = Adapto_iconv(atkGetCharset(), "UTF-8", $field['html']);
            $script .= "\$('" . str_replace('.', '_', $this->m_ownerInstance->atkEntityType() . '_' . $field['id']) . "').update(" . atkJSON::encode($value)
                    . ");\r\n";
        }
     ...
     *
     * @return Adapto_Attribute attribute instance
     */
    ...
    public function initDependencies(&$record, $fieldPrefix, $mode)
    {
        ...
        $url = partial_url($this->getOwnerInstance()->atkEntityType(), $action, "attribute." . $this->fieldName() . ".dependencies",
                array("atkdata" => array('fieldPrefix' => $fieldPrefix, 'mode' => $mode)));
        $url = atkJSON::encode($url);

        $this->getOwnerInstance()->getPage()->register_script(Adapto_Config::getGlobal('atkroot') . 'atk/javascript/class.atkattribute.js');
    ...
    protected function _callDependencies(&$record, $fieldPrefix, $mode, $initial)
    {
        ...
        $modifier = new Adapto_EditFormModifier($this->getOwnerInstance(), $record, $fieldPrefix, $mode, $initial);
    ...
    public function partial_dependencies()
    {
        ...
        foreach ($this->getOwnerInstance()->getAttributes() as $attr) {
            $attr->initDependencies($record, $fieldPrefix, $mode);
        }
    ...
    /**
     * Retrieve the attribute for the HTML-tag for this Adapto_Attribute.
     *
     ...
     *
     * @return string attribute name prefixed with entity type
     */
    function __toString()
    {
        return $this->m_ownerInstance->atkEntityType() . "::" . $this->fieldName();
    }
}
