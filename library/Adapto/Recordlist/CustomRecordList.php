 ...
 *
 */
class Adapto_Recordlist_CustomRecordList extends Adapto_RecordList
{
     ...
     */
    function render(&$entity, $recordset, $sol, $sof, $eof, $eol, $type = "0", $compression = "", $suppressList = "", $outputparams = array(), $mode = "list",
            $titlerow = true, $decode = false, $fsep = "", $rfeplace = null)
    {
        ...
        if ($titlerow) {
            ...
            foreach (array_keys($this->m_entity->m_attribList) as $attribname) {
                ...
                if (!$this->isHidden($p_attrib) && !$musthide) {
                    $output .= $sof . $this->eolreplace(atktext($p_attrib->fieldName(), $this->m_entity->m_module, $this->m_entity->m_type), $rfeplace) . $eof
                            . $fsep;
            ...
            foreach (array_keys($this->m_entity->m_attribList) as $attribname) {
                ...
                if (!$this->isHidden($p_attrib) && !$musthide) {
                    ...
                    if (method_exists($this->m_entity, $funcname)) {
                        $value = $this->eolreplace($this->m_entity->$funcname($recordset[$i], $this->m_mode), $rfeplace);
                    } else {
                    ...
                    if (atkGetCharset() != "" && $decode)
                        $value = Adapto_html_entity_decode(Adapto_htmlentities($value, ENT_NOQUOTES), ENT_NOQUOTES);
    ...
    protected function isHidden(atkAttribute $attribute)
    {
        if ($attribute->hasFlag(AF_HIDE))
            return true;
        if ($attribute->hasFlag(AF_HIDE_SELECT) && $this->m_entity->m_action === 'select')
            return true;
        if ($attribute->hasFlag(AF_HIDE_LIST) && ($this->m_entity->m_action === 'export' || $this->m_mode === 'export'))
            return true;
        return false;
    }
