<?php
  /**
   * Adapto_Document_OpenWriter class file
   *
   * @package adapto
   * @subpackage document
   *
   ...
   * @subpackage document
   */
  class Adapto_Document_OpenWriter extends Adapto_DocumentWriter
  {
     ...
     */
    public $m_tbsooo = null; // defaulted to public

    /**
     * Adapto_Document_OpenWriter Constructor
     */
    public function __construct()
    {
      $this->m_tbsooo = new clsTinyButStrongOOo;
      $this->m_tbsooo->VarPrefix = "documentvar_";
      $processdir = Adapto_Config::getGlobal("atktempdir") . "documents";
      ...
      foreach($use_vars as $key => $value)
      {
        if (is_array($value)) {
        	foreach ($value as $key => &$txtval) 
					  $txtval = Adapto_iconv(atkGetCharset(),"UTF-8",$txtval);
          $this->m_tbsooo->MergeBlock($key, $value);
        }
        else
        {
        	$value = Adapto_iconv(atkGetCharset(),"UTF-8",$value);
