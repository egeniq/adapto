...
/**
 * The Adapto_Attribute_Ml class represents an multilanguage attribute of an atkEntity.
 *
 ...
 *
 */
class Adapto_Attribute_Ml extends Adapto_Attribute
{
    public $m_language = ""; // defaulted to public
    public $m_mlattribute = TRUE; // defaulted to public
    public $m_edited = false; // defaulted to public
    ...
    public function __construct($name, $flags = 0, $size = 0)
    {
        ...
        $this->m_language = strtoupper(Adapto_Config::getGlobal("defaultlanguage"));
    }
    ...
    /**
     * Is the entity an multilanguage entity?
     *
     * @return boolean
     */
    function isMlEntity()
    {
        return ($this->m_ownerInstance->hasFlag(EF_ML));
    }
    ...
    {
        return Adapto_Config::getGlobal("supported_languages");
    }
    ...
    function edit($record = "", $fieldprefix = "", $mode = "")
    {
        ...
        for ($i = 1; $i < sizeof($languages); $i++) {
            $result .= '<input type="hidden" name="' . $fieldprefix . $this->formName() . '[' . $languages[$i] . ']"' . ' value="'
                    . htmlspecialchars($record[$this->fieldName()][$languages[$i]]) . '" >';
        }

        /* default language */
        $result .= '<input type="text" name="' . $fieldprefix . $this->formName() . '[' . $languages[0] . ']"' . ' value="'
                . htmlspecialchars($record[$this->fieldName()][$languages[0]]) . '"' . ($this->m_size > 0 ? ' size="' . $this->m_size . '"' : '')
        ...
        if ($this->m_edited) // if we edited this attrib, we also need to add the
 {
            ...
            /* label? */
            $entry["label"] = $this->label($defaults) . ' (<label id="' . $fieldprefix . $this->formName() . '_label">'
                    . atktext("language_" . strtolower($curlng)) . '</label>)';
            ...
            $entry["html"] = '<input type="text" name="' . $fieldprefix . $this->formName() . '[' . (count($this->m_languages) == 2 ? $curlng : "multilanguage")
                    ...
                    . ($this->m_size > 0 ? ' size="' . $this->m_size . '"' : '') . ($this->m_maxsize > 0 ? ' maxlength="' . $this->m_maxsize . '"' : '') . '>';
    ...
    function search($record = "", $extended = false, $fieldprefix = "")
    {
      ...
      $id .= !$this->isMlEntity()?'_'.$languages[0]:'';

      $result= '<input type="text" name="'.$id.'"'.
        ' value="' . Adapto_htmlentities($record[$this->fieldName() . (!$this->isMlEntity() ? '_' . $languages[0] : '')]) . '"'
    ...
    function hide($record = "")
    {
        ...
        for ($i = 0; $i < sizeof($languages); $i++) {
            $result .= '<input type="hidden" name="' . $this->formName() . '[' . $languages[$i] . ']"' . ' value="'
                    . htmlspecialchars($record[$this->fieldName()][$languages[$i]]) . '">';
        }
        return $result;
    }
    ...
    function addToQuery(&$query, $tablename = "", $fieldaliasprefix = "", $rec, $level, $mode)
    {
        ...
        if (!$this->isMlEntity()) {
            if ($mode == "add" || $mode == "update") {
                for ($i = 0, $_i = count($languages); $i < $_i; $i++) {
                    $query
                            ->addField($this->fieldName() . '_' . $languages[$i], $this->escapeSQL($rec[$this->fieldName()][$languages[$i]]), "", "",
            ...
            // when this entity has multilanguage support we save multiple records per occurance
    ...
    {
        $languages = $this->getLanguages();
        $attribname = $this->fieldName() . (!$this->isMlEntity() ? '_' . $languages[0] : '');
