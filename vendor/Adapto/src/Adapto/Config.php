...
class Config 
{ 
    static $s_configuration;
    
    // This will be called from Adapto's onBootstrap so we have easier access
    // to configuration values.
    static function setConfiguration($configuration)
    {
        self::$s_configuration = $configuration;
    }
    ...
    public static function get($section, $tag, $default = NULL)
    {
        ...
        if (!is_array($root)) {
            throw new \Adapto\Exception("Config section $section not found.");
        }
        ...
        if ($value != NULL) {
            ...
            if ($default === NULL) {
                throw new \Adapto\Exception("Config value '$tag' not found in section '$section' and no default provided.");
            } else {
                return $default;
            }
        }
    ...
    {        
        return self::$s_configuration[$section];
    }
