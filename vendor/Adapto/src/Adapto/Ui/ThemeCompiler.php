...
class ThemeCompiler
{
    ...
    function compile($name)
    {
        ...
        if (count($data)) {
            ...
            if (!file_exists(\Adapto\Config::get("adapto", "system.tempDir") . "/themes/")) {
                mkdir(\Adapto\Config::get("adapto", "system.tempDir") . "/themes/");
            }

            $tmpfile = new \Adapto\Util\TmpFile("themes/$name.php");
    ...
    function readStructure($name, $location = "auto")
    {
        ...
        if ($path == "") {
            // theme not found.
            $defaulttheme = \Adapto\Config::get("adapto", "theme.name");
    ...
    {        
        ...
        if (strpos($name, ".") !== false) {
            ...
            $path = \Adapto\Module::pathForModule($module) . "themes/" . $pathBase . "/";
            if (file_exists($path . "Config.php")) {
                $location = "module";
                return "module/$module/themes/$pathBase/";
            }
        } else if ($location != "app" && file_exists("vendor/Adapto/src/Adapto/Theme/$pathBase/Config.php")) {
            $location = "adapto";
            return "vendor/Adapto/src/Adapto/Theme/$pathBase/";
        }
        throw new \Adapto\Exception("Theme $name not found");
        $location = "";
        return "";
    }
    ...
    {
        $traverser = &\Adapto\ClassLoader::create("Adapto\Util\DirectoryTraverser");
            ...
            if (in_array($name, array("images", "img", "styles", "templates", "js"))) {
                ...
                foreach ($files as $file) {
                    ...
                    if (in_array($name, array("images", "img", "styles", "js"))) {
                        ...
                        if (!file_exists("public/adapto_static/".$themeName."/".$name)) {
                            if (!\Adapto\Util\Files::mkdirRecursive("public/adapto_static/".$themeName.'/'.$name)) {
                                throw new \Adapto\Exception("public/adapto_static is not writable.");
                            }
                        }           
                        
                        // Todo: move the installation of public files to a later stage, 'scan' must not have
                        // side effects.
                        copy($abspath."/".$name."/".$file, "public/adapto_static/".$themeName."/".$name."/".$file);
                    }
                ...
                foreach ($subs as $type) {
                    ...
                    foreach ($files as $file) {
                          ...
                          if (!file_exists("public/adapto_static/".$themeName."/".$name."/".$type)) {
                  
                            if (!\Adapto\Util\Files::mkdirRecursive("public/adapto_static/".$themeName.'/'.$name."/".$type)) {
                                throw new \Adapto\Exception("public/adapto_static is not writable.");
                            }
                        }           
                        ...
                        // Todo: move the installation of public files to a later stage, 'scan' must not have
                        // side effects.
                        copy($abspath."/".$name."/".$type."/".$file, "public/adapto_static/".$themeName."/".$name."/".$type."/".$file);
