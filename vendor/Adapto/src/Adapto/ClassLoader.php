...
class ClassLoader
{
     ...
     * 
     * Example:
     * Adapto\ClassLoader::mountClassPath('frontend', Adapto\Config::getGlobal('atkroot').'../frontend/');
     * Adapto\ClassLoader::create('frontend.helloworld');
     *
    ...
    static function getInstance($fullclassname, $reset = false)
    {
        ...
        if (!isset($s_instances[$fullclassname]) || $reset) {
            Util\Debugger::debug("Getting singleton instance $fullclassname");
            $s_instances[$fullclassname] = call_user_func(array($fullclassname, 'getInstance'), $reset);
        }
    ...
    function invokeFromString($str, $params = array())
    {
        ...
        if ($class != "" && $method != "") {
            $handler = &ClassLoader::create($class);
    ...
    function findClass($class)
    {
        ...
        if (!in_array($class, array_keys($classes))) {
            if (Adapto/Config::getGlobal('autoload_reindex_on_missing_class', false) && !self::$s_isReindexed) {
    ...
    {
        $traverser = ClassLoader::create('atk.utils.atkdirefctorytraverser');
        $classfinder = new ClassFinder();
        $traverser->addCallbackObject($classfinder);
        $cwd = getcwd();
        chdir(Adapto/Config::getGlobal('atkroot'));
        $traverser->traverse('atk');
        chdir($cwd);
        $classes = $classfinder->getClasses();
        Util\Debugger::debug("Adapto\Util\ClassLoader::findAllClasses(): Found " . count($classes) . ' classes');
        return $classes;
    }
}
