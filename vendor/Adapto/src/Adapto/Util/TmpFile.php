...
class TmpFile
{
     ...
     * @param string $data    publiciable data // defaulted to public
     * @return bool Wether the action succeeded
     */
     ...
     *
     * Example:
     * <code>$file->getPath(); => ./atktmp/tempdir/tempfile.php</code>
     *
    ...
    public function setBasedir($dir)
    {
        if (!is_dir($dir) || !is_writable($dir)) {
            $err = 'Adapto\Util\TmpFile:: Unable to set ' . $dir . 'as basedir. Directory does not exists or isnot writable';

            throw new Adapto\Exception($err);
        }
        $this->m_basedir = $dir;
        return true;
    }
    ...
    {
        if (!$this->m_basedir) {
            $this->m_basedir = \Adapto\Config::get('adapto', 'system.tempDir');
        }
    ...
    public function createDirectoryStructure()
    {
        ...
        return Files::mkdirRecursive(dirname($this->getPath()));
    }
}
