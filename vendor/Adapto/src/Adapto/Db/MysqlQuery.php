 ...
 *
 */
class Adapto_Db_MysqlQuery extends Adapto_Query
{
    ...
    {
        if ($value[0] == '!') {
            return $field . " NOT REGEXP '" . substr($value, 1, Adapto_strlen($value)) . "'";
        } else {
    ...
    {
        if ($value[0] == '!') {
            return "soundex($field) NOT like concat('%',substring(soundex('" . substr($value, 1, Adapto_strlen($value)) . "') from 2),'%')";
        } else {
