 ...
 *
 */
class Adapto_Db_Oci8 extends Adapto_Db
{
    /**
     * @var String
     * @access private
     */
    public $m_type = "oci8"; // defaulted to public
    public $m_vendor = "oracle"; // defaulted to public
    ...
    function query($query, $offset = -1, $limit = -1)
    {
        ...
        if ($offset >= 0 && $limit > 0) {
            /* row id's start at 1! */
            $query = "SELECT * FROM (SELECT rownum AS rid, XX.* FROM (" . $query . ") XX) YY  WHERE YY.rid >= " . ($offset + 1) . " AND YY.rid <= "
                    . ($offset + $limit);
        }
    ...
    function metadata($table, $full = false)
    {
        ...
        foreach ($rows as $i => $row) {
            ...
            $meta[$i]['flags'] = ($row['is_primary'] == 1 ? MF_PRIMARY : 0) | ($row['is_unique'] == 1 ? MF_UNIQUE : 0)
                    | ($row['is_not_null'] == 1 ? MF_NOT_NULL : 0) | ($row['is_primary'] == 1 && $row['is_auto_inc'] == 1 ? MF_AUTO_INCREMENT : 0);
