 ...
 * @abstract
 */
class Adapto_Db_Query
{
    /**
     * Array with Fieldnames
     */
    public $m_fields; // defaulted to public

    /**
     * Array with expressions.
     */
    public $m_expressions; // defaulted to public

    /**
     * Array with tables
     */
    public $m_tables; // defaulted to public

    /**
     * Array with conditions
     */
    public $m_conditions; // defaulted to public
    public $m_searchconditions; // defaulted to public

    /**
     * Var with AND or OR method
     */
    public $m_searchmethod; // defaulted to public

    /**
     * Array with aliases
     */
    public $m_aliases; // defaulted to public

    /**
     * Array with field aliases
     */
    public $m_fieldaliases; // defaulted to public

    /**
     * Array with aliases from joins
     */
    public $m_joinaliases; // defaulted to public

    /**
     * Array with Joins
     */
    public $m_joins; // defaulted to public

    /**
     * Array with group by statements
     */
    public $m_groupbys; // defaulted to public

    /**
     * Array with order by statements
     */
    public $m_orderbys; // defaulted to public

    /**
     * Do we need to perform a DISTINCT query?
     */
    public $m_distinct = false; // defaulted to public

    /**
     * Do we need to fetch only a specific set of records?
     */
    public $m_offset = 0; // defaulted to public
    public $m_limit = 0; // defaulted to public

    /**
     * Array with generated aliasses
     * Oracle has a problem when aliases are too long
     */
    public $m_generatedAlias; // defaulted to public

    /**
     * The database that this query does it's thing on
     */
    public $m_db; // defaulted to public
     ...
     * @access private
     */
    public $m_quotedfields = array(); // defaulted to public
     ...
     * @access private
     */
    public $m_reservedNames = array('from', 'select', 'order', 'group', 'release', 'index', 'table'); // defaulted to public
     ...
     * @return Adapto_Db_Query The query object itself (for fluent usage)
     */
    ...
    function exactCondition($field, $value, $dbFieldType = null)
    {
        ...
        if ($value[0] == '!') {
            return "UPPER(" . $field . ")!=UPPER('" . substr($value, 1, Adapto_strlen($value)) . "')";
        } else {
    ...
    {
        if ($value[0] == '!') {
            return "UPPER(" . $field . ") NOT LIKE UPPER('%" . substr($value, 1, Adapto_strlen($value)) . "%')";
        } else {
    ...
    {
        if ($value[0] == '!') {
            return "UPPER(" . $field . ") NOT LIKE UPPER('" . str_replace("*", "%", substr($value, 1, Adapto_strlen($value))) . "')";
        } else {
    ...
    {
        if ($value[0] == '!') {
            return $field . " < '" . substr($value, 1, Adapto_strlen($value)) . "'";
        } else {
            return $field . " > '" . $value . "'";
        }
    }
     ...
     * @return Adapto_Db_Query A Query object for the appropriate database
     */
