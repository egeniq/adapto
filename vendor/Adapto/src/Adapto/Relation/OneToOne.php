...
/**
 * flags specific for Adapto_Relation_OneToOne
 */
...
/**
 * Invisibly integrate a onetoonerelation as if the fields where part of the current entity.  
 ...
 * tabs assigned for the attributes in the destination entity. This flag is only useful in
 * integration mode.
 */
 ...
 *
 * An Adapto_Relation_OneToOne defines a relation between two tables where there
 ...
 *
 */
class Adapto_Relation_OneToOne extends Adapto_Relation
{
    /**
     * The name of the referential key attribute in the target entity.
     * @access private
     * @var String
     */
    public $m_refKey = ""; // defaulted to public

    /**
     * Default Constructor
     *
     * The Adapto_Relation_OneToOne supports two configurations:
     * - Master mode: The current entity is considered the master, and the
     *                referential key pointing to the master record is in the
     *                destination entity.
     * - Slave mode: The current entity is considered the child, and the
     *               referential key pointing to the master record is in the
     *               current entity.
     ...
     * <b>Example:</b>
     * <code>
     * $this->add(new Adapto_Relation_OneToOne("child", "mymod.childentity", "parent_id"));
     * </code>
     *
     ...
     *
     * The Adapto_Relation_OneToOne displays all values from the destination
        ...
        if ($this->createDestination()) {
            if ($mode != "update" && $mode != "add") {
                ...
                if ($this->m_refKey != "") {
                    ...
                    // Foreign key is in the source entity
            ...
            // With one exception. If the foreign key is in the source entity, we also need to update
            // the refkey value.
     ...
     * destination entity (m_destinationFilter).
     * @access private
    ...
    {
        // we need to pass all values to the destination entity, so it can
        ...
        if ($this->createDestination()) {
            (isset($rec[$this->fieldName()][$this->m_destInstance->primaryKeyField()])) ? $pkval = $rec[$this->fieldName()][$this->m_destInstance
                            ->primaryKeyField()] : $pkval = NULL;
            ...
            // should return NULL.
 {
     ...
     * this attribute.
     *
     * This implementation uses the destination entity to fetch any field that
     ...
     * For the Adapto_Relation_OneToOne, the results depends on whether the
    ...
    function storageType($mode)
    {
        ...
        // Global storage type (key null is special!)
            ...
            // foreign key is in destination entity, so we must store the
            // destination AFTER we stored the master record.
            return POSTSTORE;
        } else {
            // foreign key is in source entity, so we must store the
            // relation entity first, so we can store the foreign key
     ...
     * For the Adapto_Relation_OneToOne, this depends on the presence of the
    ...
    function store(&$db, &$record, $mode)
    {
        if ($this->createDestination()) {
            ...
            if ($vars["mode"] == "edit") {
                ...
                if ($this->m_refKey == "") {
                    // Foreign key is in source entity, so we must update the record value with
    ...
    function needsUpdate($record)
    {
        return $this->m_forceupdate
                || (parent::needsUpdate($record) && $this->createDestination() && !$this->m_destInstance->hasFlag(EF_READONLY));
    }
    ...
    function &_getStoreValue(&$record)
    {
        ...
        if ($this->m_refKey != "") {
            // Foreign key is in destination entity
            if ($this->destinationHasRelation()) {
                ...
                if ($refattr->m_destination) {
                     ...
                     */
                    $vars[$this->m_refKey][$this->m_ownerInstance->primaryKeyField()] = $this->m_ownerInstance
                            ->m_attribList[$this->m_ownerInstance->primaryKeyField()]->value2db($vars[$this->m_refKey]);
                } else {
                    $vars[$this->m_refKey] = $this->m_ownerInstance->m_attribList[$this->m_ownerInstance->primaryKeyField()]->value2db($record);
                }
            }
        } else {
            // Foreign key is in source entity
     ...
     *
     * On the other side of a oneToOneRelation (in the destination entity),
     * there may be a regular atkAttribute for the referential key, or an
     * Adapto_Relation_OneToOne pointing back at the source. This method discovers
    ...
    function destinationHasRelation()
    {
        if ($this->createDestination()) {
            if (isset($this->m_refKey) && !empty($this->m_refKey)) {
                ...
                // foreign key is in the source entity. In this case, we must check the primary key
     ...
     * because in the Adapto_Relation_OneToOne, we need to implement the
    ...
    function addToEditArray($mode, &$arr, &$defaults, &$error, $fieldprefix)
    {
        ...
        /* edit */
            ...
            /* how we handle 1:1 relations normally */
                ...
                /* readonly */
                if ($this->m_destInstance->hasFlag(EF_READONLY) || ($mode == "edit" && $this->hasFlag(AF_READONLY_EDIT))
                        || ($mode == "add" && $this->hasFlag(AF_READONLY_ADD))) {
                    $this->createDestination();
                ...
                if (!is_array($arr['fields']))
                    $arr['fields'] = array();
                if (!$this->hasFlag(AF_ONETOONE_INTEGRATE) && !$this->hasFlag(AF_NOLABEL)
                        && !(count($a["fields"]) == 1 && $a["fields"][0]["name"] == $this->m_name)) {
                    /* separator and name */
                    if ($arr['fields'][count($arr['fields']) - 1]['html'] !== '-')
                        $arr["fields"][] = array("html" => "-", "tabs" => $this->m_tabs, 'sections' => $this->getSections());
                    $arr["fields"][] = array("line" => "<b>" . atktext($this->m_name, $this->m_ownerInstance->m_module, $this->m_ownerInstance->m_type)
                                    . "</b>", "tabs" => $this->m_tabs, 'sections' => $this->getSections());
                }
    ...
    function addToViewArray($mode, &$arr, &$defaults)
    {
        ...
        /* how we handle 1:1 relations normally */
            ...
            if (!$this->hasFlag(AF_ONETOONE_INTEGRATE) && !$this->hasFlag(AF_NOLABEL)
                ...
                if ($arr['fields'][count($arr['fields']) - 1]['html'] !== '-')
                    $arr["fields"][] = array("html" => "-", "tabs" => $this->m_tabs, 'sections' => $this->getSections());
                $arr["fields"][] = array("line" => "<b>" . atktext($this->m_name, $this->m_ownerInstance->m_module, $this->m_ownerInstance->m_type) . "</b>",
                        "tabs" => $this->m_tabs, 'sections' => $this->getSections());
            }
     ...
     *
     * For the Adapto_Relation_OneToOne, this method delegates the actual
     * validation of values to the destination entity.
     *
    ...
    function validate(&$record, $mode)
    {
        ...
        if ($this->hasFlag(AF_ONETOONE_ERROR) && ($mode != "add" || !$this->hasFlag(AF_HIDE_ADD)) && $this->createDestination()) {
            ...
            foreach ($record[$this->fieldName()]["atkerror"] as $error) {
                $error['tab'] = $this->hasFlag(AF_ONETOONE_RESPECT_TABS) ? $this->m_tabs[0] : $error['tab'];
     ...
     * Adapto_Relation_OneToOne adds tabs from the destination entity to the tab
    ...
    function addToListArrayHeader($action, &$arr, $fieldprefix, $flags, $atksearch, $columnConfig, atkDataGrid $grid = null, $column = '*')
    {
        ...
        if ((!$this->hasFlag(AF_ONETOONE_INTEGRATE) && $column == '*') || $column == null) {
            ...
            throw new Exception("Invalid list column {$column} for Adapto_Relation_OneToOne " . $this->getOwnerInstance()->atkEntityType() . '::'
                    . $this->fieldName());
        }
    ...
    function getSearchCondition(&$query, $table, $value, $searchmode)
    {
        if ($this->createDestination() && is_array($value)) {
            ...
            // query, hook the attributes in the destination entity onto the query
     ...
     *                                   extended search
     * @param atkEntity $entity              The entity where the field is in
     ...
     */
    function addToSearchformFields(&$fields, &$entity, &$record, $fieldprefix = "", $currentSearchMode = array())
    {
        ...
        if ($this->hasFlag(AF_ONETOONE_INTEGRATE) && $this->createDestination()) {
            ...
            foreach (array_keys($this->m_destInstance->m_attribList) as $attribname) {
                ...
                if (!$p_attrib->hasFlag(AF_HIDE_SEARCH)) {
                    $p_attrib->addToSearchformFields($fields, $entity, $record[$this->fieldName()], $prefix, $currentSearchMode[$this->fieldName()]);
                }
            }
        } else {
            parent::addToSearchformFields($fields, $entity, $record, $fieldprefix, $currentSearchMode);
        }
    }
