 ...
 *
 */
class Adapto_Attribute_MultipleFile extends Adapto_FileAttribute
{
    /** 
     * private vars  
     */
    public $m_delimiter = ";"; // defaulted to public
        ...
        for ($i = 0; $i < count($files); $i++) {
            if (is_file($this->m_dir . $files[$i])) {
                ...
                if (in_array($ext, $prev_type)) {
                    ...
                    $r .= '<a href="' . $this->m_url . $files[$i] . '" alt="' . $files[$i] . '" onclick="NewWindow(this.href,\'name\',\'' . ($imagehw[0] + 50)
                            . '\',\'' . ($imagehw[1] + 50) . '\',\'yes\');return false;">' . $files[$i] . '</a><br>';
                } else {
