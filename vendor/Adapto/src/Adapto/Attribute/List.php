...
/**
 * The Adapto_Attribute_List class represents an attribute of an entity
 ...
 *
 */
class Adapto_Attribute_List extends Adapto_Attribute
{
    /**
     * Array with options for Listbox
     */
    public $m_options = array(); // defaulted to public

    /**
     * Array with values for Listbox
     */
    public $m_values = array(); // defaulted to public

    /**
     * Array for fast lookup of what value belongs to what option.
     */
    public $m_lookup = array(); // defaulted to public

    /**
     * Array which holds the options,values and lookup array in cache
     */
    public $m_types = array(); // defaulted to public

    /**
     * Value that is used when list is empty, normally empty
     */
    public $m_emptyvalue; // defaulted to public

    /**
     * The width of the dropdown list in pixels
     * @var int
     */
    public $m_width; // defaulted to public
    ...
    public function __construct($name, $optionArray, $valueArray = "", $flags = 0, $size = 0)
    {
        ...
        if ($size == 0) {
            for ($i = 0, $_i = count($valueArray); $i < $_i; $i++) {
                $size = max($size, Adapto_strlen($valueArray[$i]));
            }
        }
...
foreach ($optionArray AS $id => $option)
    ...
    function _addEmptyListOption()
    {
        ...
        if (!$this->hasFlag(AF_LIST_NO_NULL_ITEM)
                || ($this->hasFlag(AF_OBLIGATORY)
                        ...
                        ((Adapto_Config::getGlobal("list_obligatory_null_item") && !$this->hasFlag(AF_LIST_NO_OBLIGATORY_NULL_ITEM))
                                || ($this->hasFlag(AF_LIST_OBLIGATORY_NULL_ITEM))))) {
            $ret = '<option value="' . $this->m_emptyvalue . '">' . Adapto_htmlentities($this->text(array($this->fieldName() . '_' . $text_key, $text_key)))
                    . '</option>';
        }
        return $ret;
    }
 ...
 $id = $this->getSearchFieldName($fieldprefix);
        ...
        $code = '$(\''.$id.'\').observe(\'change\', function(event) { ' .
          $grid->getUpdateCall(array('atkstartat' => 0), array(), 'Adapto.DataGrid.extractSearchOverrides') .
          ' return false; });';
        ...
        foreach ($values AS $value) {
            if (Adapto_in_array(((string) $value), $record[$this->fieldName()], true) && $record[$this->fieldName()] !== "") {
                $sel = "selected";
            } else {
                $sel = "";
            }
