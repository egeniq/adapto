 ...
 * @subpackage attributes
 */
class Adapto_Attribute_RadioDetails extends Adapto_Attribute
{
    /**
     * Options
     *
    ...
    public function edit($record, $fieldprefix, $mode = 'add')
    {
        ...
        foreach ($this->m_options as $label => $value) {
            ...
            if ($attrNames != null) {
                $url = partial_url($this->getOwnerInstance()->atkEntityType(), $mode, 'attribute.' . $this->fieldName() . '.details',
            ...
            $result .= '
        <input type="radio" class="atkradiodetailsattribute-option" name="' . $name . '" id="' . $name . '_' . $value . '" value="' . $value . '" onchange="'
                    . $onChange . '"' . $checked . '/>
            ...
            if ($attrNames != null) {
                ...
                if ($isSelected) {
                    foreach ($attrNames as $attrName) {
                        ...
                        $result .= '<blockquote>' . $attr->edit($record, $fieldprefix, $mode) . '&nbsp;' . Adapto_htmlentities($attr->getLabel($record, $mode))
                                . '</blockquote>';
                    }
                }
                $result .= '</div>';
    ...
    public function display($record, $mode = 'list')
    {
        ...
        $result = Adapto_htmlentities($this->text($options[$value]));
        ...
        if ($mode == 'view') {
            ...
            if ($attrNames != null) {
                foreach ($attrNames as $attrName) {
                    ...
                    $result .= '
            <blockquote>
              ' . (!empty($label) ? Adapto_htmlentities($label) . ':' : '') . '
