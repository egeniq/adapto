...
/**
 * Flag(s) specific for Adapto_Attribute_Radio
 */
...
/**
 * The Adapto_Attribute_Radio class represents an attribute of an entity
 ...
 *
 */
class Adapto_Attribute_Radio extends Adapto_ListAttribute
{
    // Default number of cols / rows
    public $m_amount = 1; // defaulted to public
    public $m_cols = false; // defaulted to public
    public $m_rows = false; // defaulted to public
    public $m_clickableLabel = true; // defaulted to public
     ...
     * @var array
     */
    public $m_comments = array(); // defaulted to public
    ...
    function edit($record = "", $fieldprefix = "", $mode = "")
    {
        ...
        for ($i = 0; $i < $total_items; $i++) {
            ...
            $result .= '<td><input id="' . $labelID . '" type="radio" name="' . $fieldprefix . $this->formName() . '" '
                    . $this->getCSSClassAttribute("atkradio") . ' value="' . $values[$i] . '" ' . $onchange . $sel . '>
        ' . $this->renderValue($labelID, $this->_translateValue($values[$i], $record))
                    . ($this->hasFlag(AF_DISPLAY_VERT) && $this->m_comments[$i] != '' ? $commenthtml : '') . '</td>';
            ...
            if ($this->hasflag(AF_DISPLAY_VERT)) {
                ...
                for ($j = ($items + $i); $j < $tmp_items; $j = $j + $items) {
                    ...
                    if ($values[$j] != "") {
                        $result .= '<td><input id="' . $labelID . '" type="radio" name="' . $fieldprefix . $this->formName() . '" '
                                . $this->getCSSClassAttribute("atkradio") . ' value="' . $values[$j] . '" ' . $onchange . $sel . '>
