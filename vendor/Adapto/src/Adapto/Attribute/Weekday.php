...
/**
 * Flags for Adapto_Attribute_Weekday
 */
 ...
 *
 */
class Adapto_Attribute_Weekday extends Adapto_NumberAttribute
{
    public $m_mapping = array(1 => 'monday', 2 => 'tuesday', 4 => 'wednesday', 8 => 'thursday', 16 => 'friday', 32 => 'saturday', 64 => 'sunday'); // defaulted to public

    public $m_extra = array(); // defaulted to public
    ...
    function edit($record = "", $fieldprefix = "", $mode = 'add')
    {
        ...
        for ($i = 1; $i <= $max; $i++) {
            ...
            $result .= '<span title="' . $fullWeekday . '"><input type="checkbox" id="' . $name . '" name="' . $name . '[' . $i . ']" '
                    . $this->getCSSClassAttribute("atkcheckbox") . ' value="' . $day . '" ' . $checked . '> ' . $weekday . '</span>'
                    . ($i < $max ? $separator : '');
