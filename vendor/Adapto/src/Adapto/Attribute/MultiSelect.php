...
/**
 * The atkMultiboolAttribute class represents an attribute of an entity
 ...
 *
 */
class Adapto_Attribute_MultiSelect extends Adapto_ListAttribute
{
    // number of cols
    public $m_cols;
    ...
    public function __construct($name, $optionArray, $valueArray = "", $cols = "", $flags = 0, $size = "")
    {
        ...
        if ($size == "") {
            ...
            for ($i = 0, $_i = count($valueArray); $i < $_i; $i++) {
                $size += (Adapto_strlen($valueArray[$i]) + 1); // 1 extra for the '|' symbol
            }
        }
    ...
    function edit($record = "", $fieldprefix = "", $mode = "")
    {
        ...
        for ($i = 0; $i < count($values); $i++) {
            if (!$this->hasFlag(AF_CHECK_ALL)) {
                (Adapto_in_array($values[$i], $recordvalue)) ? $sel = "checked" : $sel = "";
            } else {
                $sel = "checked";
            }

            $result .= '<td class="table" valign="top"><input type="checkbox" id="' . $id . '_' . $i . '" ' . $this->getCSSClassAttribute("atkcheckbox")
                    . ' name="' . $fieldprefix . $this->fieldName() . '[]" value="' . $values[$i] . '" ' . $sel . '>'
                    . $this->_translateValue($values[$i], $record) . '</td>';
        ...
        if (count($this->m_values) > 4 && !hasFlag($this->m_flags, AF_NO_TOGGLELINKS)) {
            return '<div align="left"><font size="-2">
                  [<a href="javascript:void(0)" onclick="profile_checkAll(\'' . $fieldprefix . $this->fieldName() . '\'); return false;">'
                    . atktext("check_all") . '</a> <a href="javascript:void(0)" onclick="profile_checkNone(\'' . $fieldprefix . $this->fieldName()
                    . '\'); return false;">' . atktext("check_none") . '</a> <a href="javascript:void(0)" onclick="profile_checkInvert(\'' . $fieldprefix
                    . $this->fieldName() . '\'); return false;">' . atktext("invert_selection") . '</a>]</font></div>';
        }
    }
    ...
    {
        return (!isset($record[$this->fieldName()]) || (!is_array($record[$this->fieldName()]))
