 ...
 * include
 */
include_once(Adapto_Config::getGlobal("atkroot") . "atk/atkbrowsertools.inc");

/**
 * The Adapto_Attribute_Text class represents an attribute of an entity
 ...
 */
class Adapto_Attribute_Text extends Adapto_Attribute
{
    // number of rows of the edit box
    public $m_rows = 10; // defaulted to public
    public $m_cols; // defaulted to public
    public $m_autoadjust; // defaulted to public
...
\$('{$id}_textarea').focus();
 ...
 \$('{$id}_textarea').onblur = function() {
    ...
    Element.hide('{$id}_textarea');
        ...
        ");
            ...
            $html = '<textarea id="' . $id . '" name="' . $id . '" wrap="soft" rows="1" cols="20" style="overflow: hidden">' . "\n"
                    . htmlspecialchars($record[$this->fieldName()]) . '</textarea>' . '<textarea id="' . $id . '_textarea" wrap="' . $this->getWrapMode()
                    . '" rows="5" cols="40" style="display: none"></textarea>';
    ...
    function doAutoAdjust($data, &$rows, &$cols)
    {
        ...
        $maxlinechars = 0;
        for ($counter = 0, $linecharacters = 0, $rowsrequired = 1; $counter < Adapto_strlen($data); $counter++, $linecharacters++) {
