...
/** flag(s) specific for Adapto_Attribute_Duration */
...
/**
 * The Adapto_Attribute_Duration is an attribute for entering a length of time.
 *
 * @author ijansch
 * @package adapto
 * @subpackage attributes
 */
class Adapto_Attribute_Duration extends Adapto_Attribute
{
    /** member vars **/
    public $m_resolution_min; // defaulted to public
    public $m_maxtime_min; // defaulted to public
    ...
    function edit($record = "", $fieldprefix = "", $mode = "")
    {
        ...
        if (!$this->hasFlag(AF_DURATION_STRING)) {
            ...
            $result = '<input type="text" name="' . $fieldprefix . $this->formName() . '" value="' . $curval . '"'
    ...
    function _minutes2string($minutes)
    {
        ...
        if (Adapto_Config::getGlobal("durationformat", 0) == DURATIONFORMAT_DECIMAL) {
            $decimalvalue = $this->_getHourPart($minutes) + (self::_getMinutePart($minutes) / 60);
            return $prefix . sprintf("%02.02f", $decimalvalue);
        } elseif (Adapto_Config::getGlobal("durationformat", 0) == DURATIONFORMAT_TIME) {
