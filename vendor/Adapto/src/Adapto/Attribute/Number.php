...
/**
 * The Adapto_Attribute_Number can be used for numeric values.
 *
 * @author ijansch
 * @package adapto
 * @subpackage attributes
 *
 */
class Adapto_Attribute_Number extends Adapto_Attribute
{
    public $m_size; // Integral part of number (before decimal separator) // defaulted to public
    public $m_decimals = null; // The number of decimals of the number. // defaulted to public
    public $m_minvalue = false; // The minimum value of the number. // defaulted to public
    public $m_maxvalue = false; // The maximum value of the number. // defaulted to public
    public $m_use_thousands_separator = false; // use the thousands separator when formatting a number // defaulted to public
    public $m_decimalseparator; // defaulted to public
    public $m_thousandsseparator; // defaulted to public
    ...
    public function __construct($name, $flags = 0, $size = 10, $decimals = null)
    {
    ...
    function fetchMeta($metadata)
    {
        ...
        if ($this->m_searchsize > 0) {
            $this->m_searchsize = min($searchSize, $this->m_size + $this->m_decimals + 1, 20);
        }

        if ($this->m_maxsize > 0) {
            $this->m_maxsize = min($maxSize, $this->m_size, 20);
        }
    }
    ...
    function edit($record = "", $fieldprefix = "", $mode = "")
    {
        ...
        $result = '<input type="text" id="' . $id . '" class="' . get_class($this) . '" name="' . $id . '" value="' . $value . '"'
                . ($size > 0 ? ' size="' . $size . '"' : '') . ($maxsize > 0 ? ' maxlength="' . $maxsize . '"' : '') . ' ' . $onchange . ' />';
    ...
    function search($record = "", $extended = false, $fieldprefix = "")
    {
        ...
        if (!$extended) {
            ...
            $this->registerKeyListener($id, KB_CTRLCURSOR | KB_UPDOWN);
            $result .= " (" . atktext("until") . ' <input type="text" id="' . $id . '" class="' . get_class($this) . '" name="' . $id . '" value="'
                    . atk_htmlentities($value) . '"' . ($this->m_searchsize > 0 ? ' size="' . $this->m_searchsize . '"' : '') . '>)';
            return $result;
        }
    }
    ...
    function getSearchCondition(&$query, $table, $value, $searchmode)
    {
        ...
        if ($searchmode != "between") {
            ...
            if (!array_key_exists('from', $value) || strlen(trim($value['from'])) == 0)
                return false;
